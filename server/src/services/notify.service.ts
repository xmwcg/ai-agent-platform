...
export type NotifyChannel = 'disabled' | 'console' | 'wechat' | 'sms' | 'email';
...
export function resolveChannel(env: NodeJS.ProcessEnv = process.env): NotifyChannel {
  ...
  if (configured === 'disabled' || configured === 'console' || configured === 'wechat' || configured === 'sms' || configured === 'email') {
    return configured;
  }
  return 'disabled';
}
...
async function sendEmail(msg: NotifyMessage): Promise<NotifyResult> {
