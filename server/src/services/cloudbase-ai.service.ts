...
// 直连 SDK 因上游依赖存在高危漏洞已在生产禁用；文本请求统一走受控云函数代理。
...
// ================================================================
//  jymkj-knowlage SDK 安全门禁
//  上游 SDK 的 axios/lodash 传递依赖存在高危漏洞，生产暂时禁用直连；
//  AI 对话继续使用 jymkjtools-study 云函数代理与统一网关兜底，不影响用户入口。
// ================================================================
...
export function getKnowledgeGatewayInfo() {
  return {
    ...
    env: 'jymkjtools-study',
    models: AIBAK_MODELS.text,
    disabledReason: 'direct-sdk-security-review',
  };
}
...
export async function callKnowledgeGatewayChat(
  ...
  throw new Error('JYMKJ_KNOWLEDGE_SDK_DISABLED_FOR_SECURITY');
}
