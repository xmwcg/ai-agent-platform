...
export class OutboxWorker {
  ...
  private static async handlePaymentConfirmed(event: any) {
    // payload 仅用于诊断，履约必须读取权威订单，避免周期/类型被旧事件或不可信载荷覆盖。
    const result = await fulfillOrder(event.aggregateId);
    event.status = "done" as OutboxStatus;
    event.completedAt = new Date();
    await event.save();
    logger.info(
      "outbox",
      "支付确认履约完成: orderNo=" + (result as any).order?.orderNo + ((result as any).idempotent ? " (幂等)" : "")
    );
  }
