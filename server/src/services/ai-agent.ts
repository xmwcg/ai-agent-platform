import { aiModelManager, DEFAULT_TEXT_AI_MODEL_ID, DEFAULT_TEXT_AI_PROVIDER, type AIProvider } from '../config/ai-models';
...
class AIAgentService {
  ...
  async createSession(userId: string, provider?: AIProvider): Promise<string> {
    ...
    const defaultProvider = provider || DEFAULT_TEXT_AI_PROVIDER;
    ...
    const session: ChatSession = {
      ...
      model: provider
        ? (aiModelManager.getProvider(provider)?.defaultModel || DEFAULT_TEXT_AI_MODEL_ID)
  ...
  async sendMessage(
    ...
    message: string,
    config?: AgentConfig,
    modelOverride?: { model?: string; provider?: GatewayProviderName; tier?: 'free' | 'paid' }
  ): Promise<{ reply: string; usage?: any; provider?: string; model?: string }> {
    ...
    try {
      ...
      return { reply, usage, provider: result.provider, model: result.model };
    } catch (error: any) {
    ...
    try {
      ...
      for (const key of keys) {
        ...
        if (data) {
          ...
          // 旧 Redis 会话统一切到 Agnes 2.5，避免部署后继续沿用历史默认模型。
          session.provider = DEFAULT_TEXT_AI_PROVIDER;
          session.model = DEFAULT_TEXT_AI_MODEL_ID;
