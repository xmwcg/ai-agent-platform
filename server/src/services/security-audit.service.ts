 ...
 */
import { SecurityAuditLog, AuditAction, ISecurityAuditLog } from "../models/SecurityAuditLog";
 ...
 */
export async function writeAuditLog(entry: AuditEntry, critical = false): Promise<void> {
  try {
    ...
    const msg = `security audit write failed: ${(err as Error)?.message}`;
    logger.error("security-audit", msg);
    if (critical) {
...
export function writeAuditLogAsync(entry: AuditEntry): void {
  writeAuditLog(entry, false).catch((err) => {
    logger.error("security-audit", `async audit write failed: ${err?.message}`);
  });
}

/** 根据操作类型自动分类严重级别 */
function classifySeverity(action: AuditAction): ISecurityAuditLog["severity"] {
  const criticals: AuditAction[] = ["key_rotate", "key_view", "secret_access", "sandbox_security_event"];
  const highs: AuditAction[] = ["role_change", "permission_change", "refund_approve", "ledger_adjust", "account_delete", "admin_action"];
  const mediums: AuditAction[] = ["login_failed", "login_blocked", "password_change", "mfa_remove", "session_revoke", "api_key_delete"];
  if (criticals.includes(action)) return "critical";
  if (highs.includes(action)) return "high";
  if (mediums.includes(action)) return "medium";
  return "low";
}
...
export async function queryAuditLogs(params: {
  ...
  const [records, total] = await Promise.all([
    SecurityAuditLog.find(filter).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).lean(),
