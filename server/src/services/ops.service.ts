...
export async function getOpsSnapshot(): Promise<OpsSnapshot> {
  ...
  const revenueTrend: OpsSnapshot['revenueTrend'] = [];
  for (let i = 11; i >= 0; i--) {
    ...
    const paidInMonth = await User.countDocuments({
      plan: { $in: ['pro', 'max', 'team'] },
      createdAt: { $lt: to },
    });
    revenueTrend.push({
      month: `${from.getFullYear()}-${String(from.getMonth() + 1).padStart(2, '0')}`,
      mrr: (agg[0]?.total ?? 0) / 100,
      paidUsers: paidInMonth,
    });
  }

  // --- 12-month signup trend ---
  const signupTrend: OpsSnapshot['signupTrend'] = [];
  for (let i = 11; i >= 0; i--) {
    const from = new Date(new Date().getFullYear(), new Date().getMonth() - i, 1);
    const to = new Date(new Date().getFullYear(), new Date().getMonth() - i + 1, 1);
  ...
  // --- Plan breakdown ---
  const planBreakdown: OpsSnapshot['planBreakdown'] = [];
