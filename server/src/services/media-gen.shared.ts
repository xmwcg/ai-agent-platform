...
export interface MediaGenResult {
  ...
  thumbnailUrl?: string;
  duration?: number;
  /** 长视频父任务关联的片段任务，按最终拼接顺序保存。 */
  segmentTaskIds?: string[];
  provider: string;
  note: string;
}
...
export async function persistTask(taskId: string, result: MediaGenResult, options?: { segmentTaskIds?: string[] }): Promise<void> {
  try {
    if (MediaTask.db.readyState === 1) {
      await MediaTask.findOneAndUpdate(
        { taskId },
        {
          ...
          note: result.note,
          ...(options?.segmentTaskIds ? { segmentTaskIds: options.segmentTaskIds } : {}),
        },
    ...
    if (MediaTask.db.readyState === 1) {
      ...
      if (doc) {
        return {
          ...
          note: doc.note,
          ...(doc.segmentTaskIds?.length ? { segmentTaskIds: doc.segmentTaskIds } : {}),
