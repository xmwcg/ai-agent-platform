import { SecretAuditLog, SecretAuditAction } from '../models/SecretAuditLog';
 ...
 */
export async function logSecretAudit(entry: SecretAuditEntry): Promise<void> {
  try {
    ...
    logger.error('secret-audit', '写入密钥审计日志失败（已忽略，不阻塞主流程）', (e as Error).message);
  }
}
...
const TEST_WINDOW_MS = 60_000; // 滑动窗口 60 秒
 ...
 */
export function checkTestAbuse(actorId: string, ip: string): boolean {
  const now = Date.now();
  ...
  if (arr.length > TEST_MAX) {
    logger.warn(
      'secret-audit',
      `高频测试连接告警：actor=${actorId} ip=${ip} 在 ${TEST_WINDOW_MS / 1000}s 内触发 ${arr.length} 次`
    );
    return true;
  }
  return false;
