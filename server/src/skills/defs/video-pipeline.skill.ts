 ...
 * 这些模型不具备对话能力，必须回退到对话型通道，否则调研/脚本阶段会直接失败。
 *
 * 关键修复：对话阶段优先使用已验证可用的免费文本通道 agnes（与成片通道一致）。
 * 原因：① cloudbase(hy3) 免费额度频控极严（连发即 403），直接绑定会导致整条流水线失败；
 *       ② deepseek 余额不足(402)；③ zhipu/mc 等通道响应慢，被路由 3.5s 超时误杀。
 * 只有 agnes 免费且稳定可用，因此对话阶段落到 agnes，成片阶段也用 agnes 视频模型，
 * 端到端可生成。若用户明确选择了其他可用的对话型 Provider，则尊重用户选择。
 */
function resolveChatProvider(provider?: string): string {
  if (!provider) return 'agnes';
  if (VIDEO_PROVIDER_RE.test(provider)) return 'agnes';
  if (provider === 'cloudbase') return 'agnes'; // hy3 频控严重，交给 agnes
  return provider;
}
...
async function callChatStage(
  ...
  const attempt = (t: number) =>
    route({ ...(chatProvider ? { provider: chatProvider as any } : {}), messages, maxTokens, temperature: t, timeoutMs: 60000 });
...
export const videoPipelineSkill: Skill = {
  ...
  async invoke(ctx) {
    const { topic, duration, style, compose = true, provider, size, frameRate } = ctx.input || {};
      ...
      if (compose) {
        ...
        try {
          composeResult = await mediaGenService.generate({
            ...
            ...(frameRate ? { frameRate: Number(frameRate) } : {}),
            ...(style ? { style } : {}),
          } as any);
          ...
          // 成片失败（如 Agnes 视频队列满 video_queue_full / 上游 503）不应拖垮整条流水线：
          // 调研 / 脚本 / 视觉提示词已真实生成，降级保留，并明确提示成片稍后重试。
          composeError = `成片阶段暂未成功：${ce?.message || ce}（脚本与视觉提示词已生成，可稍后重试成片）`;
          console.warn('[video-pipeline] compose degraded:', composeError);
        }
      }
      ...
      return {
        ...
        data: {
          stages: {
            ...
            ...(composeError ? { composeStatus: 'failed' as const, composeError } : {}),
            ...(!compose ? { composeStatus: 'skipped' as const } : {}),
          },
          next: composeResult
