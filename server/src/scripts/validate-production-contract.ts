...
export function createStaticProductionContractEnv(): NodeJS.ProcessEnv {
  ...
  return {
    ...
    TRANSYNC_BASE_URL: 'https://translate.aibak.invalid',
    TRANSYNC_SSO_CLIENT_SECRET: 'ci-contract-transync-sso-placeholder-'.padEnd(64, 'x'),
...
if (require.main === module) {
  try {
    ...
    console.error(
      `生产配置契约校验失败：${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
