...
// importKnowledgeBase 模块独立管理，必要时单独提交；播种时内联5篇示例文档
 ...
 */
export async function seedKnowledgeSamples(): Promise<void> {
  let n = 0;
  for (const s of SAMPLES) {
    const dup = await KnowledgeDocument.findOne({ title: s.title });
    if (dup) continue;
  ...
  if (n > 0) {
    logger.info('seed', `已播种 ${n} 篇示例知识文档`);
  } else {
    logger.info('seed', '示例知识文档已存在，跳过播种');
  }
  // 同步导入「金奕鸣通用知识库」Markdown 内容库（需要 importKnowledgeBase 模块先提交）
  // await seedKnowledgeBase();
}

// 允许独立运行：ts-node --transpile-only src/scripts/seedKnowledge.ts [--dry-run]
