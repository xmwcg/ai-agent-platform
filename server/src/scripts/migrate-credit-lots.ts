...
async function main(): Promise<void> {
  ...
  if (!uri) throw new Error('缺少 MONGODB_URI，拒绝执行额度迁移');

  logger.info('credit-migration', DRY_RUN ? '额度迁移演练启动' : '额度迁移启动', {
    batch: BATCH,
    limit: LIMIT || 'all',
  ...
  const query = User.find({}).select('_id credits').sort({ _id: 1 });
  ...
  for await (const user of cursor) {
    ...
    const existingMarker = await CreditLot.findOne({ userId, idempotencyKey }).lean();
    if (existingMarker) {
      alreadyTracked++;
      continue;
    }

    const existingLots = await CreditLot.find({
      userId,
      status: 'active',
      ...
      .lean();
    const lotBalance = existingLots.reduce(
      (sum, lot) => sum + Number(lot.remainingAmount || 0),
      0
    );
    ...
    try {
      await session.withTransaction(async () => {
        const freshUser = await User.findById(userId).select('credits').session(session);
        if (!freshUser || Number(freshUser.credits || 0) !== cachedBalance) {
          throw new Error('迁移期间用户余额发生变化');
        }
        const lotCount = await CreditLot.countDocuments({ userId }).session(session);
        if (lotCount !== 0) throw new Error('迁移期间已出现额度批次');
  ...
  logger.info('credit-migration', '额度迁移结束', {
    dryRun: DRY_RUN,
    batch: BATCH,
    scanned,
    migratable,
  ...
  if (failures.length > 0) {
    for (const item of failures.slice(0, 20)) {
      logger.error('credit-migration', `用户 ${item.userId} 迁移失败: ${item.reason}`);
    }
    throw new Error(`额度迁移存在 ${failed} 个失败用户，已拒绝以成功状态退出`);
  }

  console.log(
    `✅ 额度迁移${DRY_RUN ? '演练' : ''}完成：扫描 ${scanned}，可迁移 ${migratable}，已迁移 ${migrated}，已跟踪 ${alreadyTracked}，零余额 ${zeroBalance}`
  );
}
...
main()
  ...
  .catch(async (error) => {
    logger.error('credit-migration', '脚本失败', error instanceof Error ? error.message : String(error));
    try {
      await mongoose.disconnect();
    } catch {
