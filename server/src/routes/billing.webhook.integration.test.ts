...
beforeEach(async () => {
  ...
  await User.updateOne(
    { _id: testUserId },
    {
      $set: { plan: 'free', subscriptionCancelAtPeriodEnd: false, subscriptionCanceledAt: null },
      $unset: { membershipExpiresAt: 1 },
    }
  );
});
...
// ═══════════════════════════════════════════════════════
// E2E-6: ProjectGrade 购买上下文 → Webhook → 权益即时生效
// ═══════════════════════════════════════════════════════

describe('E2E-6: ProjectGrade 支付商业闭环', () => {
  jest.setTimeout(20000);

  function stripeEvent(orderNo: string, transactionId: string) {
    const eventBody = JSON.stringify({
      type: 'payment_intent.succeeded',
      data: { object: { id: transactionId, metadata: { orderNo } } },
    });
    const ts = Math.floor(Date.now() / 1000).toString();
    const sig = crypto
      ...
      .update(`${ts}.${eventBody}`)
      .digest('hex');
    return { eventBody, signature: `t=${ts},v1=${sig}` };
  }

  it('智评通下单幂等 → Stripe Webhook → Pro 权益实时生效 → 重放不重复履约', async () => {
    const initial = await request(app)
      .get('/api/project-grade/entitlements')
      .set('Authorization', `Bearer ${token}`);
    expect(initial.status).toBe(200);
    expect(initial.body.data.entitlements.plan.id).toBe('free');
    expect(initial.body.data.entitlements.plan.upgradeUrl).toBe(
    ...
    const purchase = {
      ...
      idempotencyKey: `pg-${crypto.randomUUID()}`,
    };
    const orderRes = await request(app)
      .post(`${BASE}/orders`)
      .set('Authorization', `Bearer ${token}`)
      .send(purchase);
    expect(orderRes.status).toBe(200);
    expect(orderRes.body.data.sourceProduct).toBe('project_grade');
    expect(orderRes.body.data.returnTo).toBe('/project-grade/projects');
    const { orderNo } = orderRes.body.data;
    ...
    expect(retryRes.body.data.orderNo).toBe(orderNo);
    expect(retryRes.body.data.idempotent).toBe(true);

    const conflictRes = await request(app)
      .post(`${BASE}/orders`)
      .set('Authorization', `Bearer ${token}`)
      .send({ ...purchase, plan: 'max' });
    expect(conflictRes.status).toBe(409);
    expect(conflictRes.body.code).toBe('ORDER_IDEMPOTENCY_CONFLICT');

    const event = stripeEvent(orderNo, `pi_pg_${Date.now()}`);
    const webhook = await request(app)
      .post(`${BASE}/webhook/stripe`)
      .set('Content-Type', 'application/json')
      .set('stripe-signature', event.signature)
      .send(event.eventBody);
    expect(webhook.status).toBe(200);
    expect(webhook.body.fulfillmentPending).toBe(false);

    const entitlementsRes = await request(app)
      .get('/api/project-grade/entitlements')
      .set('Authorization', `Bearer ${token}`);
    expect(entitlementsRes.status).toBe(200);
    const entitlements = entitlementsRes.body.data.entitlements;
    expect(entitlements.plan.id).toBe('pro');
    expect(entitlements.projects.limit).toBe(5);
    expect(entitlements.daily.project_grade_source_scan.limit).toBe(5);
    expect(entitlements.daily.project_grade_evaluation.limit).toBe(10);
    expect(entitlements.daily.project_grade_report_publish.limit).toBe(3);
    expect(entitlements.daily.project_grade_report_download.limit).toBe(10);
    expect(entitlements.capabilities).toMatchObject({
    ...
    const { Order } = require('../models/Order');
    const { User } = require('../models/User');
    const paidOrder = await Order.findOne({ orderNo }).lean();
    ...
    const beforeReplay = await User.findById(testUserId).select('credits membershipExpiresAt').lean();
    ...
    const afterReplay = await User.findById(testUserId).select('credits membershipExpiresAt').lean();
    expect(afterReplay?.credits).toBe(beforeReplay?.credits);
    expect(afterReplay?.membershipExpiresAt?.toISOString()).toBe(beforeReplay?.membershipExpiresAt?.toISOString());

    const { OutboxEvent } = require('../models/OutboxEvent');
    expect(await OutboxEvent.countDocuments({ aggregateId: orderNo, eventType: 'payment_confirmed' })).toBe(1);
  });

  it('产品专用订阅保存套餐 id，并兼容旧前端字段', async () => {
    const idempotencyKey = `zpt-${crypto.randomUUID()}`;
    const canonical = await request(app)
      ...
      .send({
        ...
        period: 'monthly',
        provider: 'mock',
        sourceProduct: 'zhipingtong',
        productPackageId: 'zpt_enterprise',
        idempotencyKey,
      });

    expect(canonical.status).toBe(200);
    const { Order } = require('../models/Order');
    const canonicalOrder = await Order.findOne({ orderNo: canonical.body.data.orderNo }).lean();
    expect(canonicalOrder).toMatchObject({
      sourceProduct: 'zhipingtong',
      packageId: 'zpt_enterprise',
      plan: 'team',
    });

    const changedPackage = await request(app)
      .post(`${BASE}/orders`)
      .set('Authorization', `Bearer ${token}`)
      .send({
        plan: 'team',
        ...
        provider: 'mock',
        sourceProduct: 'zhipingtong',
        productPackageId: 'zpt_team',
        idempotencyKey,
      });
    expect(changedPackage.status).toBe(409);
    expect(changedPackage.body.code).toBe('ORDER_IDEMPOTENCY_CONFLICT');
    ...
    const legacy = await request(app)
      ...
      .send({
        ...
        sourceProduct: 'zhipingtong',
        packageId: 'zpt_enterprise',
        idempotencyKey: `zpt-legacy-${crypto.randomUUID()}`,
      });

    expect(legacy.status).toBe(200);
    const legacyOrder = await Order.findOne({ orderNo: legacy.body.data.orderNo }).lean();
    expect(legacyOrder?.packageId).toBe('zpt_enterprise');
    ...
    const conflict = await request(app)
      ...
      .send({
        ...
        productPackageId: 'zpt_enterprise',
        packageId: 'zpt_team',
        idempotencyKey: `zpt-conflict-${crypto.randomUUID()}`,
      });
    expect(conflict.status).toBe(400);
    expect(conflict.body.code).toBe('PRODUCT_PACKAGE_ID_CONFLICT');
  });

  it('拒绝跨域 returnTo，并让年付订单按真实周期履约', async () => {
    ...
    expect(invalid.body.code).toBe('INVALID_BILLING_RETURN_TO');
    ...
    const event = stripeEvent(orderNo, `pi_pg_year_${Date.now()}`);
    await request(app)
      .post(`${BASE}/webhook/stripe`)
      .set('Content-Type', 'application/json')
      .set('stripe-signature', event.signature)
    ...
    const { User } = require('../models/User');
    const user = await User.findById(testUserId).select('membershipExpiresAt').lean();
    const remainingDays = (user!.membershipExpiresAt!.getTime() - Date.now()) / 86400000;
    expect(remainingDays).toBeGreaterThan(360);
  });

  it('private_license 走独立签发，不会错误激活平台订阅', async () => {
    const { Order } = require('../models/Order');
    const { User } = require('../models/User');
    const { fulfillPaidOrder } = require('../services/billing-order-fulfillment.service');
    const orderNo = `PRIVATE${Date.now()}`;
    await Order.create({
      orderNo,
      userId: testUserId,
      orderType: 'private_license',
      packageId: 'ent-standard',
      plan: 'free',
      period: 'monthly',
      amount: 29900,
      currency: 'CNY',
    ...
    await fulfillPaidOrder(orderNo);
    const user = await User.findById(testUserId).select('plan membershipExpiresAt').lean();
    expect(user?.plan).toBe('free');
    expect(user?.membershipExpiresAt).toBeUndefined();
    const fulfilled = await Order.findOne({ orderNo }).lean();
    expect(fulfilled?.fulfillmentStatus).toBe('fulfilled');
    expect(fulfilled?.licensePayload?.fingerprint).toBe('PENDING_ACTIVATION');
    expect(fulfilled?.licensePayload?.sign).toMatch(/^[A-F0-9]{64}$/);
  });

  it('取消续订保留已付费周期权益，并且重复请求保持幂等', async () => {
    const orderRes = await request(app)
      .post(`${BASE}/orders`)
      .set('Authorization', `Bearer ${token}`)
      .send({ plan: 'pro', period: 'monthly', provider: 'mock' });
    ...
    const paymentRes = await request(app)
      .get(`${BASE}/orders/${orderRes.body.data.orderNo}/pay`)
      .set('Authorization', `Bearer ${token}`);
    ...
    const beforeCancel = await request(app)
      .get(`${BASE}/subscription`)
      .set('Authorization', `Bearer ${token}`);
    expect(beforeCancel.body.data.plan).toBe('pro');
    expect(beforeCancel.body.data.cancelAtPeriodEnd).toBe(false);
    const originalExpiresAt = beforeCancel.body.data.membershipExpiresAt;

    const cancel = await request(app)
      .post(`${BASE}/subscription/cancel`)
      .set('Authorization', `Bearer ${token}`);
    ...
    expect(afterCancel.body.data).toMatchObject({
      ...
      expired: false,
      cancelAtPeriodEnd: true,
      membershipExpiresAt: originalExpiresAt,
    });
    ...
    expect(repeatedCancel.body.data).toMatchObject({
      cancelAtPeriodEnd: true,
      originalExpiresAt,
    });
  });
