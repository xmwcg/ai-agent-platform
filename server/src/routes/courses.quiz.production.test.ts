...
describe('课程测验真实数据与答案保护', () => {
  ...
  it('GET 从课程数据库结果返回测验，但不泄漏正确答案和解析', async () => {
    ...
    expect(select).toHaveBeenCalledWith('title chapters isPublished price requiredPlan freePreviewChapters');
  ...
  it('免费用户不能通过测验 URL 绕过锁定章节', async () => {
    mockQuizLookup(createCourse({ price: 39900, requiredPlan: 'pro', freePreviewChapters: 0 }));
    ...
    expect(response.body.code).toBe('COURSE_CHAPTER_LOCKED');
