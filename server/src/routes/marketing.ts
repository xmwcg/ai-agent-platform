...
请生成促单建议。`;
    ...
    // 调用全平台默认 Agnes 2.5；失败由 generateText 内部回退免费通道。
...
router.post('/enterprise-inquiry', async (req: Request, res: Response) => {
  try {
    const { company, contact, email, phone, requirements, planInterest } = req.body;
    if (!contact || !requirements) {
      return res.status(400).json({ error: '请填写联系人和需求描述' });
    }
    // 存储咨询记录
    const inquiry = { company, contact, email, phone, requirements, planInterest, createdAt: new Date(), status: 'pending' };
    logger.info('marketing', '企业咨询提交', { company, contact, planInterest });
    res.json({ success: true, message: '咨询已提交，我们将在 24 小时内联系您' });
  } catch (error: any) {
    sendError(res, error);
  }
});
