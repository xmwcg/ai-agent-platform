...
import { aiModelManager, getPreferredAgnesTextModel } from '../config/ai-models';
...
const CUSTOMER_SERVICE_RAG_TIMEOUT_MS = 5_000;
const CUSTOMER_SERVICE_PROVIDER_TIMEOUT_MS = 7_000;
const CUSTOMER_SERVICE_AI_TOTAL_TIMEOUT_MS = 18_000;

export function withCustomerServiceTimeout<T>(promise: Promise<T>, ms: number, label: string): Promise<T> {
...
router.post('/', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const preferredModel = getPreferredAgnesTextModel();
    const preferredSeparator = preferredModel.indexOf('/');
    const preferredProvider = preferredSeparator > -1 ? preferredModel.slice(0, preferredSeparator) : 'agnes25';
    const preferredModelId = preferredSeparator > -1 ? preferredModel.slice(preferredSeparator + 1) : preferredModel;
    ...
    const cs = await CustomerService.create({
      ...
      provider: provider || preferredProvider || aiModelManager.getDefaultProvider()?.name.toLowerCase() || 'agnes25',
      csModel: model || preferredModelId || aiModelManager.getDefaultProvider()?.defaultModel || 'agnes-2.5-flash',
      welcomeMessage,
      fallbackMessage,
      ownerId: req.user!.id,
...
router.post('/chat/:embedCode', optionalAuth, enforceQuota('cs_query'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (cs.knowledgeBaseIds.length > 0) {
      ...
      if (docs.length > 0) {
        const searchResults = await withCustomerServiceTimeout(
          embeddingService.searchSimilarDocuments(message, { limit: 5, minSimilarity: 0.6 }),
          CUSTOMER_SERVICE_RAG_TIMEOUT_MS,
          '知识库检索'
        ).catch(() => [] as ScoredDoc[]);
    ...
    if (escalated) {
      ...
      if (mockMode) {
        ...
        // 历史版本的内置默认客服统一升级到 Agnes 2.5；用户显式配置的自定义/其他厂商模型继续保留。
        const storedModel = cs.csModel.includes('/') ? cs.csModel : `${cs.provider}/${cs.csModel}`;
        const legacyDefaultProviders = new Set(['openai', 'deepseek', 'agnes', 'cloudbase']);
        const storedProvider = storedModel.split('/', 1)[0]?.toLowerCase();
        const fullModel = legacyDefaultProviders.has(storedProvider)
        ...
        try {
          const result = await route({
            ...
            temperature: 0.5,
            timeoutMs: CUSTOMER_SERVICE_PROVIDER_TIMEOUT_MS,
            totalTimeoutMs: CUSTOMER_SERVICE_AI_TOTAL_TIMEOUT_MS,
          });
    ...
    // 5. 计数、配额与合规审计相互独立，并行落库，减少模型返回后的额外等待。
    await Promise.all([
      CustomerService.findByIdAndUpdate(cs._id, { $inc: { conversationCount: 1 } }),
      req.user?.id ? quotaIncrement(req.user.id, 'cs_query') : Promise.resolve(),
      CustomerServiceAuditLog.create(
        buildAuditEntry({
