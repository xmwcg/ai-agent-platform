...
/** OAuth Mock 只能在测试或本机回环地址使用，公网请求一律 fail-closed。 */
function canUseOAuthMock(req: Request, providerMockEnabled: boolean): boolean {
  if (!providerMockEnabled) return false;
  if (process.env.NODE_ENV === 'test') return true;
  const hostname = req.hostname.trim().toLowerCase().replace(/^\[|\]$/g, '');
...
router.post('/register', authLimiter, validate(registerSchema), async (req: AuthRequest, res: Response) => {
  try {
    const { email, password, name, referralCode, acceptTerms, acceptPrivacy, attributionSessionId } = req.body;
...
router.get('/login-methods', async (req: Request, res: Response) => {
  ...
  res.json({
    ...
    data: {
      ...
      wechatMock: canUseOAuthMock(req, OAUTH_CONFIG.wechat.mock),
      douyinMock: canUseOAuthMock(req, OAUTH_CONFIG.douyin.mock),
    },
  });
});
...
router.get('/wechat/qr', async (req: Request, res: Response) => {
  try {
    ...
    const mock = canUseOAuthMock(req, cfg.mock);
    if (!cfg.enabled && !mock) {
    ...
    const state = crypto.randomBytes(8).toString('hex');
    await redisClient.set(`wechat:state:${state}`, '1', 'EX', 600);
    const authorizeUrl = `${cfg.authorizeUrl}?appid=${cfg.clientId}&redirect_uri=${redirectUri}&response_type=code&scope=${cfg.scope}&state=${state}#wechat_redirect`;
...
router.get('/wechat/callback', async (req: Request, res: Response) => {
  try {
    ...
    const cfg = OAUTH_CONFIG.wechat;
    const mock = canUseOAuthMock(req, cfg.mock);
    if ((!cfg.enabled && !mock) || (code === 'mock' && !mock)) {
...
router.get('/douyin/qr', async (req: Request, res: Response) => {
  try {
...
router.get('/douyin/h5', async (req: Request, res: Response) => {
  try {
    const cfg = OAUTH_CONFIG.douyin;
    const mock = canUseOAuthMock(req, cfg.mock);
    if (!cfg.enabled && !mock) {
