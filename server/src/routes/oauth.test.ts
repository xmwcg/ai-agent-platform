...
describe('公网 OAuth Mock 防护', () => {
  ...
  it('公网 Host 不返回任何 Mock 登录方式', async () => {
    const res = await request(mount()).get('/login-methods').set('Host', 'aibak.site');
    expect(res.status).toBe(200);
    expect(res.body.data.wechatMock).toBe(false);
    expect(res.body.data.douyinMock).toBe(false);
  });

  it.each(['/wechat/qr', '/douyin/qr', '/douyin/h5'])(
    '公网 Host 访问 %s 时未配置凭据必须返回 503',
    async (path) => {
      const res = await request(mount()).get(path).set('Host', 'aibak.site');
