...
beforeAll(async () => {
  ...
  process.env.DEFAULT_PAY_PROVIDER = 'mock';

  mongoServer = await MongoMemoryReplSet.create({ replSet: { count: 1, storageEngine: "wiredTiger" }, instanceOpts: [{ launchTimeout: 60000 }] });
