...
import { enforceAnonymousFreeCap } from '../middleware/subscription';
 ...
 */
router.post('/chat', optionalAuth, enforceAnonymousFreeCap('ai_chat'), async (req: AuthRequest, res: Response) => {
  try {
    const { message, messages: inputMessages, model, stream = false, history } = req.body;
    const requestedModel = String(model || getPreferredAgnesTextModel());
    ...
    let text: string;
    let provider = 'agnes25';
    let usedModel = requestedModel;
    const gatewayModel = requestedModel === 'agnes-2.5-flash'
      ? 'agnes25/agnes-2.5-flash'
      : requestedModel;

    // 全平台文本默认通道：Agnes 2.5。旧 hy3 / 其他免费模型仍保留为手动选择和失败回退。
    if (gatewayModel.startsWith('agnes25/')) {
      try {
        const gw = await route({
          model: gatewayModel,
        ...
        logger.warn('aibak-chat', `Agnes 2.5 失败，回退 CloudBase hy3: ${agnesErr?.message || '未知'}`);
        text = await callCloudbaseChat(chatMessages, 'hy3');
        provider = 'cloudbase-free-fallback';
        usedModel = 'hy3';
      }
    } else if (isKnowledgeGatewayAvailable()) {
      try {
        text = await callKnowledgeGatewayChat(chatMessages, requestedModel);
        provider = 'jymkj-knowlage-gateway';
      } catch (gatewayErr: any) {
        logger.warn('aibak-chat', `知识网关失败，回退 CloudBase: ${gatewayErr.message}`);
        text = await callCloudbaseChat(chatMessages, requestedModel);
      ...
      try {
        text = await callCloudbaseChat(chatMessages, requestedModel);
        provider = 'jymkjtools-study-proxy';
      } catch (cloudbaseErr: any) {
        logger.warn('aibak-chat', `CloudBase 失败，回退 Agnes 2.5: ${cloudbaseErr?.message || '未知'}`);
        const gw = await route({
...
router.get('/status', async (_req, res: Response) => {
  const textModels = [getPreferredAgnesTextModel(), ...AIBAK_MODELS.text];
  const models = [...textModels, ...AIBAK_MODELS.image];
