...
export interface QuickstartTemplate {
  ...
  desc: string;
  icon: string;
  category: 'generic' | 'industry'; // generic=通用场景；industry=行业零技术接入垂直包
  vertical?: string; // 行业标识：clinic / law / training / factory ...
  knowledge: { title: string; content: string; tags: string[] }[];
  bot: { name: string; systemPrompt: string; welcomeMessage: string };
...
export const QUICKSTART_TEMPLATES: QuickstartTemplate[] = [
  {
    ...
    modelHint: '默认使用 Agnes 2.5 Flash；管理员仍可手动切换其他已配置模型',
  },
  {
    id: 'media-ops',
    name: '新媒体运营助手',
 ...
 */
export const INDUSTRY_TEMPLATES: QuickstartTemplate[] = [
  {
    id: 'clinic',
    name: '诊所智能导诊',
...
/** 合并通用 + 行业模板，作为完整模板目录 */
export const ALL_QUICKSTART_TEMPLATES: QuickstartTemplate[] = [
...
router.get('/templates', (req: Request, res: Response) => {
  ...
  const list = category
    ? ALL_QUICKSTART_TEMPLATES.filter((t) => t.category === category)
    : ALL_QUICKSTART_TEMPLATES;
  res.json({
    success: true,
...
router.post('/apply', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const tpl = ALL_QUICKSTART_TEMPLATES.find((t) => t.id === templateId);
    if (!tpl) return res.status(404).json({ success: false, error: '模板不存在' });

    // 归属团队时校验：必须是该团队成员（>= member）
    let validatedTeamId: string | undefined;
    if (teamId) {
    ...
    const docs = await KnowledgeDocument.insertMany(
      tpl.knowledge.map((k) => ({
        title: k.title,
        content: k.content,
        tags: k.tags,
    ...
    const cs = await CustomerService.create({
      name: tpl.bot.name,
      knowledgeBaseIds: docs.map((d: any) => d._id.toString()),
      systemPrompt: tpl.bot.systemPrompt,
      welcomeMessage: tpl.bot.welcomeMessage,
      provider: DEFAULT_TEXT_AI_PROVIDER,
      csModel: DEFAULT_TEXT_AI_MODEL_ID,
      ownerId: req.user!.id,
      teamId: validatedTeamId,
      embedCode: crypto.randomBytes(12).toString('hex'),
      // 行业模板预置合规触发词：命中即转人工（诊所急诊 / 律所起诉 / 工厂起火等）
      escalationTriggers: tpl.escalationTriggers || [],
    });
    res.json({
      success: true,
      data: { knowledgeIds: docs.map((d: any) => d._id), customerServiceId: cs._id },
    });
  } catch (err) {
    sendError(res, err);
  }
