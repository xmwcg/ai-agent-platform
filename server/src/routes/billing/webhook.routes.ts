...
import { Order, PaymentProvider, PaymentStatus } from "../../models/Order";
...
router.post("/webhook/:provider", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 先写唯一 Outbox，再尝试即时履约。即时履约失败不回滚“已支付”，由 Worker 最终恢复。
    await ensurePaymentConfirmedOutbox(order.orderNo, { provider, transactionId: result.transactionId });
    let fulfillmentPending = false;
    try {
      await (fulfillOrder as any)(order.orderNo);
    } catch (error) {
      fulfillmentPending = true;
      logger.error("webhook", `支付已确认但即时履约失败，将由 Outbox 重试: orderNo=${order.orderNo}`, error);
    }
