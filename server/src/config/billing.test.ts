...
describe('billing plan logic', () => {
  ...
  it('plans define daily quota limits', () => {
    ...
    expect(Object.keys(PLANS)).toEqual(['free', 'pro', 'max', 'team', 'enterprise']);
  });
  ...
  it('every plan defines the full quota resource set', () => {
    ...
    for (const id of ['free', 'pro', 'max', 'team', 'enterprise'] as ReadonlyArray<PlanId>) {
  ...
  it('defines complete ProjectGrade commercial entitlements for every plan', () => {
    const planIds: PlanId[] = ['free', 'pro', 'max', 'team', 'enterprise'];
    for (const id of planIds) {
      expect(getPlan(id).projectGrade).toEqual(
        expect.objectContaining({
          activeProjects: expect.any(Number),
          reportPublishEnabled: expect.any(Boolean),
          reportDownloadEnabled: expect.any(Boolean),
          reportValidityDays: expect.any(Number),
          removeAibakBranding: expect.any(Boolean),
        })
      );
    }

    expect(getPlan('free').limits.project_grade_source_scan).toBe(0);
    expect(getPlan('free').limits.project_grade_report_publish).toBe(0);
    expect(getPlan('free').limits.project_grade_report_download).toBe(0);
    expect(getPlan('free').projectGrade.reportPublishEnabled).toBe(false);
    expect(getPlan('pro').projectGrade.reportPublishEnabled).toBe(true);
  });

  it('ProjectGrade limits and capacities do not decrease on higher plans', () => {
    const ordered: PlanId[] = ['free', 'pro', 'max', 'team', 'enterprise'];
    ...
    for (let index = 1; index < ordered.length; index += 1) {
      const previous = getPlan(ordered[index - 1]);
      const current = getPlan(ordered[index]);
      const normalizeUnlimited = (value: number) => value === -1 ? Number.POSITIVE_INFINITY : value;
      expect(normalizeUnlimited(current.projectGrade.activeProjects))
        .toBeGreaterThanOrEqual(normalizeUnlimited(previous.projectGrade.activeProjects));
      for (const resource of resources) {
        expect(normalizeUnlimited(current.limits[resource]))
          .toBeGreaterThanOrEqual(normalizeUnlimited(previous.limits[resource]));
      }
    }
    expect(getPlan('team').limits.project_grade_report_download).toBe(-1);
  });
...
describe('PLAN_AI_BUDGET_FEN · 日 AI 成本预算', () => {
  ...
  it('all plans have a budget entry', () => {
    const expected: PlanId[] = ['free', 'pro', 'max', 'team', 'enterprise'];
