...
export function collectStartupEnvErrors(env: EnvLike = process.env): string[] {
  ...
  for (const key of ['MONGODB_URI', 'REDIS_URL', 'PUBLIC_BASE_URL'] as const) {
    if (!present(env, key)) errors.push(`${key} 未配置`);
  }
  if (present(env, 'PUBLIC_BASE_URL') && !isHttpsUrl(env.PUBLIC_BASE_URL)) {
    errors.push('PUBLIC_BASE_URL 必须是有效的 HTTPS 地址');
  }

  if (!present(env, 'TRANSYNC_BASE_URL')) {
    errors.push('TRANSYNC_BASE_URL 未配置');
  } else if (!isHttpsUrl(env.TRANSYNC_BASE_URL)) {
    errors.push('TRANSYNC_BASE_URL 必须是有效的 HTTPS 独立站点地址');
  }
  const transyncSsoSecret = env.TRANSYNC_SSO_CLIENT_SECRET?.trim() || '';
  if (transyncSsoSecret.length < 48) {
    errors.push('TRANSYNC_SSO_CLIENT_SECRET 必须是至少 48 个字符的强随机密钥');
  }
  ...
  if (!notifyChannel) {
    ...
    for (const key of ['WECHAT_OPEN_APPID', 'WECHAT_OPEN_SECRET', 'WECHAT_NOTIFY_TEMPLATE_ID'] as const) {
      if (!present(env, key)) errors.push(`${key} 未配置`);
    }
  }

  // ─── 邮件渠道配置 ───
  const emailTransport = env.EMAIL_TRANSPORT?.trim().toLowerCase();
  if (!emailTransport || emailTransport === 'mock') {
    // 生产环境不强制要求邮件（非关键路径），但给出提示
  } else if (emailTransport === 'sendgrid') {
    if (!present(env, 'SENDGRID_API_KEY')) {
      errors.push('EMAIL_TRANSPORT=sendgrid 时 SENDGRID_API_KEY 未配置');
    }
    if (!present(env, 'EMAIL_FROM')) {
      errors.push('EMAIL_FROM（发件人地址）未配置');
    }
  } else if (emailTransport === 'smtp') {
    for (const key of ['SMTP_HOST', 'SMTP_PORT', 'SMTP_USER', 'SMTP_PASS'] as const) {
      if (!present(env, key)) errors.push(`EMAIL_TRANSPORT=smtp 时 ${key} 未配置`);
    }
  }

  // ─── 金网通 Store 联动配置 ───
  const jwtStoreUrl = env.JINWANGTONG_STORE_URL?.trim();
  if (jwtStoreUrl) {
    if (!present(env, 'JINWANGTONG_ADMIN_TOKEN')) {
      errors.push('JINWANGTONG_STORE_URL 已配置但 JINWANGTONG_ADMIN_TOKEN 未配置');
    }
  }
...
export function assertStartupEnv(env: EnvLike = process.env): void {
...
export function validateStartupEnv(env: EnvLike = process.env): void {
  ...
  if (WEAK_JWT_SECRETS.has(secret) || secret.length < 48) {
    ...
    console.warn(
      '⚠️ JWT_SECRET 为弱占位值或长度不足；开发环境继续运行，生产环境会拒绝启动。'
    );
  }
}
