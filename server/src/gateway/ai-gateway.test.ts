...
import {
  ...
  getEffectiveProviderTimeout,
} from './ai-gateway.service';

describe('AI 网关 - 总耗时预算', () => {
  it('按剩余总预算压缩单 provider 超时，预算耗尽后立即停止', () => {
    expect(getEffectiveProviderTimeout(7_000, 18_000, 0)).toBe(7_000);
    expect(getEffectiveProviderTimeout(7_000, 18_000, 15_500)).toBe(2_500);
    expect(getEffectiveProviderTimeout(7_000, 18_000, 18_000)).toBe(0);
    expect(getEffectiveProviderTimeout(7_000, undefined, 99_000)).toBe(7_000);
  });
});
...
describe('AI 网关 - Agnes 2.5 增量接入', () => {
  ...
  it('保留旧 Agnes 模型并新增 agnes-2.5-flash', () => {
    const providers = listGatewayProviders();
    expect(providers).toEqual(expect.arrayContaining([
      expect.objectContaining({ name: 'agnes', configured: true }),
      expect.objectContaining({ name: 'agnes25', configured: true }),
    ]));

    const models = listGatewayModels();
    expect(models.find((item) => item.provider === 'agnes')?.models).toEqual(
      expect.arrayContaining(['agnes-2.0-flash', 'agnes-image-2.0-flash', 'agnes-video-v2.0'])
    );
    expect(models.find((item) => item.provider === 'agnes25')?.models).toEqual(['agnes-2.5-flash']);
  });

  it('免费公开调用可显式路由到 Agnes 2.5', async () => {
    ...
    expect(result.provider).toBe('agnes25');
    expect(result.model).toBe('agnes-2.5-flash');
    expect(result.reply).toBe('CUSTOM_OK');
  });
});
