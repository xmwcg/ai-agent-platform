...
/**
 * 匿名免费档防刷闸门：对未登录用户的 AI 对话做「按 IP 的日维度次数上限」，
 * 防止白嫖 / 脚本击穿平台垫付成本与免费额度频控。
 *
 * - 已登录用户由 enforceQuota / enforceCostValve 管理，本中间件直接放行。
 * - 计数基于 Redis 日维度键，天然滚动清零，无需定时任务。
 * - Redis 异常时放行（不阻断主流程），保证可用性优先。
 */
export function enforceAnonymousFreeCap(resource: QuotaResource = 'ai_chat') {
  const dailyLimit = Number(process.env.ANON_DAILY_CHAT_LIMIT) || 30;
  return async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    if (req.user) return next(); // 已登录走自身配额/成本阀门
    try {
      const ip = (req.ip || 'unknown').replace(/[:.]/g, '_');
      const key = `anoncap:${ip}:${resource}:${todayKey()}`;
      const used = Number(await redisClient.get(key)) || 0;
      if (used >= dailyLimit) {
        res.status(402).json({
          error: '今日免费体验次数已用尽',
          code: 'ANON_FREE_LIMIT',
          resource,
          limit: dailyLimit,
          used,
          message: '免费体验次数已用完。登录即可领取每日免费额度，或升级套餐 / 改用自带 Key（BYOK）零成本继续。',
          upgradeUrl: '/pricing',
          loginUrl: '/login',
        });
        return;
      }
      // 预占：本次放行后累加（并发场景可能小幅超出上限，可接受）
