  ...
  async increment(key: string): Promise<{ totalHits: number; resetTime: Date | undefined }> {
    ...
    try {
      ...
      if (count === 1) {
        ...
        (realRedis.pexpire(full, Math.ceil(this.windowMs)) as Promise<number>).catch(
          () => undefined
        );
      }
...
// 智评通匿名公开体检：每 IP 5 次/小时。测试也启用，确保限流契约有集成测试覆盖。
const publicProjectGradeScanWindowMs =
  Number(process.env.PROJECT_GRADE_PUBLIC_SCAN_RATE_WINDOW_MS) || 60 * 60 * 1000;
export const publicProjectGradeScanLimiter = rateLimit({
  windowMs: publicProjectGradeScanWindowMs,
  max: Number(process.env.PROJECT_GRADE_PUBLIC_SCAN_RATE_MAX) || 5,
  keyGenerator: (req) => req.ip || req.socket.remoteAddress || 'unknown',
  standardHeaders: true,
  legacyHeaders: false,
  store: makeStore(publicProjectGradeScanWindowMs, 'rl:project-grade-public-scan'),
  handler: (_req, res) => {
    res.status(429).json({
      success: false,
      error: '免费体检每小时最多 5 次，请稍后再试',
      code: 'PROJECT_GRADE_PUBLIC_URL_SCAN_RATE_LIMITED',
    });
  },
});
