...
import { seedProjectGradeBaseline } from './scripts/seed-project-grade-baseline';
...
import projectGradePublicRoutes from './routes/project-grade.public';
...
import transyncIntegrationRoutes from './routes/transync-integration';
import flowRoutes from './routes/flow';
import adminKnowledgeProductsRoutes from './routes/admin-knowledge-products';
import apiKeyRoutes from './routes/api-keys';
import jinwangtongRoutes from './jinwangtong/jinwangtong.routes';
import { mcpService } from './services/mcp.service';
import { sendError } from './lib/http-error';
import { logger } from './lib/logger';
import { DEFAULT_TEXT_AI_MODEL_ID, DEFAULT_TEXT_AI_PROVIDER } from './config/default-ai-model';
...
import { projectGradeProjectionRecoveryWorker } from './services/project-grade-projection-recovery.service';
...
process.on('unhandledRejection', (reason: unknown) => {
  logger.error(
    'index',
    `⚠ 未处理的 Promise 拒绝: ${reason instanceof Error ? reason.stack : String(reason)}`
  );
});
...
// 禁止 Cloudflare/CDN 缓存所有 API 响应
app.use('/api', (_req: Request, res: Response, next: NextFunction) => {
  res.set('Cache-Control', 'no-cache, no-store, must-revalidate, max-age=0');
  res.set('CDN-Cache-Control', 'no-cache, no-store');
...
app.use('/api/admin/knowledge-products', adminKnowledgeProductsRoutes);
...
app.use('/api/jinwangtong', jinwangtongRoutes);
app.use('/api/marketing', marketingRoutes);
app.use('/api/integrations/transync', transyncIntegrationRoutes);
...
app.use('/api/studio', studioRoutes); // NexMind 创作工坊（短视频/数字人/电商图文）
app.use('/api/referral', referralRoutes); // 推荐/分销体系
app.use('/api/notifications', notificationRoutes); // 站内消息中心
...
app.use(
  '/generated',
  express.static(process.env.OSS_LOCAL_DIR || path.join(__dirname, '..', 'uploads', 'generated'))
);
...
export interface BootstrapDependencies {
  ...
  seedProjectGrade: () => Promise<unknown>;
...
function scheduleDailyReconciliation(): NodeJS.Timeout {
  ...
  const runIfNeeded = async () => {
    ...
    try {
      ...
      logger.info(
        'reconciliation-cron',
        `对账完成: batchId=${result.batchId} status=${result.status} matched=${result.matchedOrders} unmatched=${result.unmatchedOrders}`
      );
    } catch (err: any) {
  ...
  logger.info(
    'reconciliation-cron',
    `每日对账定时器已注册，首次将在 ${Math.round(initialDelay / 3600000)} 小时后触发`
  );
...
async function seedDefaultRelayChannels(): Promise<void> {
  const candidates = [
    {
      name: 'Agnes 2.5 默认免费渠道（自动播种）',
      envKey: process.env.AGNES25_API_KEY ? 'AGNES25_API_KEY' : 'AGNES_25_API_KEY',
      provider: DEFAULT_TEXT_AI_PROVIDER,
      baseURL: process.env.AGNES25_BASE_URL || process.env.AGNES_25_BASE_URL || 'https://api.agnes-ai.cn/v1',
...
function defaultBootstrapDependencies(): BootstrapDependencies {
  return {
    ...
    seedProjectGrade: seedProjectGradeBaseline,
    startHttpServer: () => {
      ...
      startApmPersistence();
      projectGradeProjectionRecoveryWorker.start();
...
async function startManagedDependency(
  label: string,
  ...
  try {
    ...
    logger.warn(
      'index',
      `${label} 启动失败，非生产环境继续运行: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export async function bootstrap(options: BootstrapOptions = {}): Promise<Server | undefined> {
  ...
  await startManagedDependency('Outbox Worker', async () => {
    dependencies.startOutboxWorker();
  });
  ...
  await startManagedDependency('智评通基线报告', dependencies.seedProjectGrade);
...
if (require.main === module) {
  bootstrap().catch((error) => {
    logger.error(
      'index',
      `生产启动失败，进程退出: ${error instanceof Error ? error.stack || error.message : String(error)}`
    );
    process.exit(1);
  });
}
