...
const memoryRedis = {
    get: jest.fn((k) => Promise.resolve(__store.has(k) ? __store.get(k) : null)),
    set: jest.fn((k, v) => { __store.set(k, String(v)); return Promise.resolve('OK'); }),
    incrby: jest.fn((k, by) => {
    ...
    del: jest.fn((k) => { __store.delete(k); return Promise.resolve(1); }),
    on: jest.fn(),
