...
exports.videoPipelineSkill = void 0;
 ...
 */
function resolveChatProvider(provider) {
    if (!provider)
        return 'deepseek';
    if (VIDEO_PROVIDER_RE.test(provider))
        return 'deepseek';
    return provider;
}
/** 视觉提示词兜底：上游偶发空响应时，基于主题构造纯画面英文描述（符合视频模型内容策略）。 */
function buildFallbackVisualPrompt(topic, style) {
...
exports.videoPipelineSkill = {
    ...
    async invoke(ctx) {
        ...
        const chatProvider = resolveChatProvider(provider);
        try {
...
${research.content}`,
            ...
            if (compose) {
                ...
                const agnesReady = await (0, media_gen_service_1.ensureAgnesLoaded)();
...
//# sourceMappingURL=video-pipeline.skill.js.map