...
exports.chatSkill = {
    ...
    async invoke(ctx) {
        ...
        const providerOverride = typeof provider === 'string' ? provider : undefined;
        ...
        try {
            const sid = typeof sessionId === 'string' && sessionId.trim()
                ? sessionId.trim()
                : await ai_agent_1.aiAgentService.createSession(ctx.userId || 'anon', providerOverride);
            const result = await ai_agent_1.aiAgentService.sendMessage(sid, message.trim(), config, { ...(providerOverride ? { provider: providerOverride } : {}), ...(modelOverride ? { model: modelOverride } : {}) });
...
//# sourceMappingURL=ai-chat.skill.js.map