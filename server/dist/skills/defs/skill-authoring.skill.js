...
exports.skillAuthoringSkill = void 0;
 ...
 */
exports.skillAuthoringSkill = {
    manifest: {
        id: 'skill-authoring',
        name: '技能编写',
        description: '用 AI 网关生成新业务技能的 manifest + invoke 骨架，沉淀为可上架的 Skill 定义（superpowers writing-skills 等价物）。',
    ...
    async invoke(ctx) {
        ...
        if (!goal)
            return { ok: false, error: 'skill-authoring 需要 goal（要沉淀的能力描述）' };
...
invoke 必须调用对应 service，生产失败时显式返回错误且不得伪造结果。只输出 JSON：{ manifest: {...}, invokeSkeleton: "ts 代码字符串" }。`;
        ...
        if (draft && draft.manifest && draft.manifest.id) {
            ...
            const skeleton = typeof draft.invokeSkeleton === 'string' && draft.invokeSkeleton.trim()
        ...
        return {
            ok: true,
            data: {
                howToRegister: '将下方 TS 骨架写入 server/src/skills/defs/<id>.skill.ts，并在 registry.ts 中 import + 加入 SKILLS 数组，最后补充 skills.test.ts 断言（或运行 check:superpowers）。',
                tsFile,
                draft,
            },
        };
...
//# sourceMappingURL=skill-authoring.skill.js.map