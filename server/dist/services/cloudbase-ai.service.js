...
exports.AIBAK_MODELS = void 0;
exports.callCloudbaseChat = callCloudbaseChat;
exports.callCloudbaseImage = callCloudbaseImage;
 ...
 */
async function callCloudbaseChat(chatMessages, model = 'hy3') {
    const response = await axios_1.default.post(CLOUDBASE_CHAT_URL, { messages: chatMessages, model, stream: false }, { headers: { 'Content-Type': 'application/json' }, timeout: 60000 });
 ...
 * @throws 当云函数不可用或返回失败时
 */
async function callCloudbaseImage(model, prompt, opts = {}) {
    const isImage2Image = model.includes('I2I');
    const body = { model, prompt, size: opts.size || '1024x1024' };
    if (isImage2Image) {
        if (opts.imageBase64)
            body.imageBase64 = opts.imageBase64;
        else if (opts.imageUrl)
            body.image_urls = [opts.imageUrl];
    }
    const response = await axios_1.default.post(CLOUDBASE_IMAGE_URL, body, {
        headers: { 'Content-Type': 'application/json' },
        timeout: 170000, // 图像生成可能较长，需小于云函数 180s 超时
    });
    if (!response.data?.success)
        throw new Error(response.data?.error || 'CLOUDBASE_IMAGE_ERROR');
    const images = response.data.data ||
        response.data.images ||
        (response.data.url ? [{ url: response.data.url }] : []) ||
        [];
    return images.map((i) => i?.url || i).filter(Boolean);
}
//# sourceMappingURL=cloudbase-ai.service.js.map