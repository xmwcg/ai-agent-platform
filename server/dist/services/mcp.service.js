...
const ALLOWED_STDIO_COMMANDS = (process.env.MCP_ALLOWED_STDIO_COMMANDS || 'node,npx')
    ...
    .map((s) => s.trim())
    .filter(Boolean);
function isAllowedStdioCommand(command) {
    if (!command)
        return false;
...
function normalizeStdioCommand(command, args) {
    ...
    if (!pkg || !pkg.startsWith('@modelcontextprotocol/server-'))
        return { command, args };
    const entry = path_1.default.join(process.cwd(), 'node_modules', pkg, 'dist', 'index.js');
...
class MCPService extends events_1.EventEmitter {
    ...
    async loadFromDB() {
        try {
            const docs = await MCPServer_1.MCPServer.find().lean();
            for (const doc of docs) {
                ...
                if (norm.command !== config.command || norm.args !== config.args) {
                    logger_1.logger.info('mcp', `规范化为 node 命令: ${config.name} (${config.command} -> node)`);
            ...
            logger_1.logger.info('mcp', `Loaded ${docs.length} MCP server config(s) from database`);
        }
        catch (err) {
            logger_1.logger.warn('mcp', `Failed to load MCP servers from DB: ${err.message}`);
        }
    }
    ...
    async persist(id) {
        ...
        if (!config)
            return;
        try {
            await MCPServer_1.MCPServer.updateOne({ id }, {
                $set: {
                    id: config.id,
                    name: config.name,
            ...
            logger_1.logger.warn('mcp', `Failed to persist MCP server: ${err.message}`);
        }
    }
    // 注册服务器配置
    ...
    async removeServer(id) {
        ...
        try {
            await MCPServer_1.MCPServer.deleteOne({ id });
        }
        catch (err) {
            logger_1.logger.warn('mcp', `Failed to delete MCP server from DB: ${err.message}`);
        }
    }
    // 设置启用状态
    ...
    async connectStdio(id) {
        ...
        if (!isAllowedStdioCommand(config.command)) {
            ...
            logger_1.logger.error('mcp', `stdio 命令 "${config.command}" 不在白名单，拒绝连接（允许：${ALLOWED_STDIO_COMMANDS.join(', ')}）`);
        ...
        try {
            ...
            const client = new index_js_1.Client({ name: 'ai-agent-platform', version: '0.1.0' }, { capabilities: {} });
            ...
            this.servers.set(id, config);
            logger_1.logger.info('mcp', `MCP server connected: ${config.name} (${toolsResult.tools?.length || 0} tools)`);
            ...
            logger_1.logger.error('mcp', `MCP connect failed: ${config.name}`, err?.message);
    ...
    async connectSSE(id) {
        ...
        try {
            ...
            this.clients.set(id, client);
            this.servers.set(id, config);
            logger_1.logger.info('mcp', `MCP SSE connected: ${config.name}`);
            this.emit('connected', id, config);
        }
        catch (err) {
            config.status = 'error';
            this.servers.set(id, config);
            logger_1.logger.error('mcp', `MCP SSE connect failed: ${config.name}`, err.message);
            throw err;
        }
    }
    // 连接服务器（自动判断类型）
    ...
    async callTool(serverId, toolName, args) {
        ...
        try {
            ...
            logger_1.logger.error('mcp', `MCP tool call failed: ${serverId}/${toolName}`, err.message);
            throw err;
        }
    }
