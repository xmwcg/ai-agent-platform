...
exports.sandboxService = exports.DANGEROUS_PATTERNS = exports.SUPPORTED_LANGUAGES = void 0;
...
exports.selectSandboxMode = selectSandboxMode;
exports.buildLocalCommand = buildLocalCommand;
/**
...
exports.SUPPORTED_LANGUAGES = ['python', 'javascript', 'typescript', 'bash'];
/** 语言别名归一化（可被单测） */
...
function detectDangerousPatterns(code) {
    if (typeof code !== 'string')
        return [];
    const hits = [];
    for (const { label, re } of exports.DANGEROUS_PATTERNS) {
        if (re.test(code))
            hits.push(label);
    }
    return hits;
...
function sanitizeOutput(output, maxBytes = 64 * 1024) {
...
function selectSandboxMode(explicit, config = process.env) {
    ...
    if (explicit && ['mock', 'local', 'remote'].includes(explicit)) {
        if (explicit === 'local' && !localEnabled)
            return 'mock';
        return explicit;
    }
    if (cfg.mode === 'local')
        return localEnabled ? 'local' : 'mock';
    if (cfg.mode === 'remote' && cfg.remoteUrl && cfg.remoteToken)
        return 'remote';
    return 'mock';
}
/** 纯函数：为本机执行构建命令与参数（local 模式使用），含 Python 隔离模式 -I */
function buildLocalCommand(language, filePath, bins = { pythonBin: 'python3', nodeBin: 'node' }) {
    switch (language) {
        case 'python':
...
function genExecutionId() {
    return `sbx_${Date.now()}_${(0, crypto_1.randomBytes)(3).toString('hex')}`;
}
/** 模拟执行：回显代码中可识别的 print/console.log 字面量，并标注演示模式 */
async function simulateOutput(code, language) {
    const lines = ['[演示模式] 沙盒未配置真实执行环境，以下为模拟输出。'];
    if (language === 'python') {
...
class LocalProvider {
    ...
    async run(req, ctx) {
        ...
        const filePath = (0, path_1.join)((0, os_1.tmpdir)(), `sbx_${(0, crypto_1.randomBytes)(6).toString('hex')}${fileExt}`);
        try {
            await (0, promises_1.writeFile)(filePath, req.code, 'utf8');
            ...
            await (0, promises_1.rm)(filePath, { force: true }).catch(() => undefined);
        }
    }
}
function runWithTimeout(cmd, args, timeoutMs) {
    return new Promise((resolve, reject) => {
        const child = (0, child_process_1.execFile)(cmd, args, { timeout: timeoutMs, maxBuffer: 64 * 1024 * 1024 }, (err, stdout, stderr) => {
            if (err?.killed) {
...
class RemoteProvider {
    ...
    async run(req, ctx) {
        ...
        try {
            ...
            const resp = await axios_1.default.post(url, { language: req.language, code: req.code }, {
...
exports.sandboxService = {
    ...
    async checkRateLimit(userId) {
        const now = Date.now();
        const minuteKey = `sandbox:ratelimit:${userId}:minute`;
        const dayKey = `sandbox:ratelimit:${userId}:day`;
        const concurrentKey = `sandbox:ratelimit:${userId}:concurrent`;
        const circuitKey = `sandbox:ratelimit:${userId}:circuit-breaker`;
        try {
            const circuitUntil = await database_1.realRedis.get(circuitKey);
            if (circuitUntil) {
                const until = parseInt(circuitUntil, 10);
                if (until > now) {
                    return { allowed: false, reason: '执行频率过高，请稍后再试（熔断保护）', retryAfterMs: until - now };
                }
                await database_1.realRedis.del(circuitKey);
            }
            const pipeline = database_1.realRedis.pipeline();
            pipeline.incr(concurrentKey);
            pipeline.expire(concurrentKey, 30);
            const concurrentResults = await pipeline.exec();
            const currentConcurrent = concurrentResults?.[0]?.[1];
            if (currentConcurrent !== undefined && currentConcurrent > 3) {
                await database_1.realRedis.decr(concurrentKey);
                return { allowed: false, reason: '并发执行数已达上限（3），请等待当前任务完成' };
            }
            const minuteCount = await database_1.realRedis.incr(minuteKey);
            if (minuteCount === 1)
                await database_1.realRedis.expire(minuteKey, 60);
            if (minuteCount > 10) {
                await database_1.realRedis.decr(concurrentKey);
                return { allowed: false, reason: '每分钟最多 10 次执行', retryAfterMs: 60000 };
            }
            const dayCount = await database_1.realRedis.incr(dayKey);
            if (dayCount === 1) {
                const s = Math.ceil((new Date().setHours(24, 0, 0, 0) - now) / 1000);
                await database_1.realRedis.expire(dayKey, s);
            }
            if (dayCount > 200) {
                await database_1.realRedis.decr(concurrentKey);
                await database_1.realRedis.decr(minuteKey);
                return { allowed: false, reason: '每日最多 200 次执行', retryAfterMs: 86400000 };
            }
            return { allowed: true };
        }
        catch (redisErr) {
    ...
    async releaseConcurrentSlot(userId) { try {
        await database_1.realRedis.decr(`sandbox:ratelimit:${userId}:concurrent`);
    }
    catch { /* ignore */ } },
    async recordFailure(userId) {
        try {
            const key = `sandbox:ratelimit:${userId}:failures:24h`;
            const count = await database_1.realRedis.incr(key);
            if (count === 1)
                await database_1.realRedis.expire(key, 86400);
            if (count >= 20) {
                await database_1.realRedis.set(`sandbox:ratelimit:${userId}:circuit-breaker`, String(Date.now() + 300000), 'EX', 300);
            }
        }
        catch { /* ignore */ }
    },
    async persistExecution(userId, result, code) {
        try {
            const codeHash = (0, crypto_1.createHash)('sha256').update(code, 'utf8').digest('hex');
            await SandboxExecution_1.SandboxExecution.create({ executionId: result.executionId, userId, language: result.language, codeHash, codeLength: Buffer.byteLength(code, 'utf8'), mode: result.mode, status: result.status, stdout: result.stdout?.slice(0, 4096), stderr: result.stderr?.slice(0, 4096), exitCode: result.exitCode, durationMs: result.durationMs, deniedPatterns: result.deniedPatterns, resourceUsage: result.resourceUsage, securityEvents: result.securityEvents });
            ...
            console.error('sandbox persistExecution error:', err?.message || err);
        }
    },
};
exports.default = exports.sandboxService;
