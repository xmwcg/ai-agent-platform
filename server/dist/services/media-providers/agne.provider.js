...
function toValidFrames(durationSec, frameRate) {
...
class AgneProvider {
    ...
    async generate(params) {
        ...
        if (type === 'text2img' || type === 'image2image') {
            ...
            if (type === 'image2image' && params.imageUrl)
                body.image_url = params.imageUrl;
            const resp = await axios_1.default.post(`${AGNES_BASE_URL}/images/generations`, body, {
                headers,
                timeout: 120000,
            });
        ...
        // 图生视频：传参考图 URL
        if (type === 'image2video' && params.imageUrl)
            body.image = params.imageUrl;
        if (params.negativePrompt)
            body.negative_prompt = params.negativePrompt;
        const resp = await axios_1.default.post(`${AGNES_BASE_URL}/videos`, body, { headers, timeout: 120000 });
        const d = resp.data || {};
        // 官方推荐用 video_id 轮询；缺省回退 task_id / id
        const vendorId = String(d.video_id || d.task_id || d.id || (0, media_gen_shared_1.genTaskId)());
        const result = {
            type,
            taskId: vendorId,
            status: 'processing',
        ...
        await (0, media_gen_shared_1.persistTask)(vendorId, result);
        return result;
    }
    ...
    async queryTask(taskId) {
        ...
        let d = {};
        try {
            const resp = await axios_1.default.get(`${AGNES_ROOT}/agnesapi`, {
                headers,
            ...
            const resp = await axios_1.default.get(`${AGNES_BASE_URL}/videos/${encodeURIComponent(taskId)}`, {
                headers,
                timeout: 30000,
            });
            d = resp.data || {};
...
exports.AgneProvider = AgneProvider;
//# sourceMappingURL=agne.provider.js.map