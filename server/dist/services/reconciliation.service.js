...
class ReconciliationService {
     ...
     * 参数 date: 对账日期，不传则对昨天
     */
    static async triggerReconciliation(date) {
        ...
        // 检查是否已有同批对账
        const existing = await ReconciliationRecord_1.ReconciliationRecord.findOne({ batchId });
        if (existing && existing.status !== "pending") {
            logger_1.logger.info("reconciliation", `批次 ${batchId} 已完成，跳过`);
            return existing;
        }
        const record = await ReconciliationRecord_1.ReconciliationRecord.create({
            batchId,
            provider: "wechat",
        ...
        try {
            // 1. 查询系统订单（当日已支付）
            const systemOrders = await Order_1.Order.find({
            ...
            try {
                ...
                logger_1.logger.error("reconciliation", `下载微信账单失败: ${message}`);
            ...
            const message = err instanceof Error ? err.message : String(err);
            logger_1.logger.error("reconciliation", `对账流程异常: ${message}`);
            record.status = "unmatched";
            record.errorMessage = message;
            record.completedAt = new Date();
        }
        await record.save();
        logger_1.logger.info("reconciliation", `对账完成: batchId=${batchId} status=${record.status} matched=${record.matchedOrders} unmatched=${record.unmatchedOrders}`);
        return record;
    }
    /**
     * 下载微信交易账单
     * 使用微信 v3 交易账单下载接口
     */
    static async downloadWeChatBill(date) {
        ...
        try {
            const applyResp = await axios_1.default.get(`https://api.mch.weixin.qq.com${applyUrl}`, {
                headers: {
            ...
            if (!downloadUrl) {
                logger_1.logger.warn("reconciliation", "账单申请成功但无 download_url");
                return [];
            }
            // 下载账单
            const billResp = await axios_1.default.get(downloadUrl, {
                responseType: "text",
                timeout: 30000,
            });
            ...
            if (err.response?.status === 404) {
                logger_1.logger.info("reconciliation", "当日无微信交易记录");
                return [];
            }
            throw err;
        }
     ...
     */
    static async autoRetryUnmatched(differences, batchId) {
        for (const diff of differences) {
            if (diff.type === "missing_in_wechat") {
                // 检查是否有 webhook 事件记录
                const event = await WebhookEvent_1.WebhookEvent.findOne({
                ...
                if (event) {
                    ...
                    logger_1.logger.info("reconciliation", `差异 ${diff.orderNo} 已通过 webhook 补全`);
                }
            }
        }
    }
     ...
     * 查询对账列表
     */
    static async getReconciliationList(page, limit) {
        const [records, total] = await Promise.all([
            ReconciliationRecord_1.ReconciliationRecord.find()
                .sort({ batchId: -1 })
                .skip((page - 1) * limit)
                .limit(limit)
                .lean(),
     ...
     * 查询对账详情
     */
    static async getReconciliationDetail(batchId) {
        const record = await ReconciliationRecord_1.ReconciliationRecord.findOne({ batchId }).lean();
        if (!record)
            throw new http_error_1.AppError(404, "对账批次不存在", "RECONCILIATION_NOT_FOUND");
        return record;
    }
}
