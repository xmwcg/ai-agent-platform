...
exports.XHS_AGENTS = void 0;
exports.listXhsAgents = listXhsAgents;
exports.generateXhsCopy = generateXhsCopy;
...
function listXhsAgents() {
    return exports.XHS_AGENTS.map(({ id, name, description }) => ({ id, name, description }));
}
function buildUserPrompt(input) {
    ...
    if (audience)
        lines.push(`目标受众：${audience}`);
    if (style)
        lines.push(`风格/语气：${style}`);
    if (keywords)
        lines.push(`关键词/补充卖点：${keywords}`);
    if (role === 'copywriter') {
...
function tryParseStructured(raw) {
    ...
    try {
        ...
        if (Array.isArray(obj.hashtags))
            result.hashtags = obj.hashtags.filter((x) => typeof x === 'string');
        if (Array.isArray(obj.imageSuggestions))
            result.imageSuggestions = obj.imageSuggestions.filter((x) => typeof x === 'string');
 ...
 */
async function generateXhsCopy(input) {
    const agent = exports.XHS_AGENTS.find((a) => a.id === input.role);
    if (!agent) {
...
//# sourceMappingURL=xhs-copy.service.js.map