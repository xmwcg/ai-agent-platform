...
const rag_pipeline_types_1 = require("./rag-pipeline.types");
// ── 智能分块器 ────────────────────────────────────────
class DocumentChunker {
    constructor(config = {}) {
        this.config = { ...rag_pipeline_types_1.DEFAULT_CONFIG, ...config };
    }
    /**
     * 基于段落 + 语义边界的分块策略
     *
    ...
    chunk(text) {
        ...
        logger_1.logger.info('rag-pipeline', `Chunked into ${chunks.length} chunks (avg ${Math.round(text.length / Math.max(chunks.length, 1))} chars/chunk)`);
        return chunks;
    }
}
exports.DocumentChunker = DocumentChunker;
//# sourceMappingURL=rag-pipeline.chunker.js.map