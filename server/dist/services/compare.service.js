...
exports.compareService = exports.CompareService = void 0;
...
class CompareService {
    ...
    async generateCompare(req) {
        const items = this.presets.filter(p => req.items.includes(p.id));
        if (items.length < 2) {
            throw new Error('At least 2 items required');
        }
        const dimensions = this.getDimensions(items[0].type);
        try {
            return await this.generateCompareWithAI(items, dimensions);
        }
        catch (err) {
            logger_1.logger.warn('compare', `AI 对比生成失败: ${err.message}`);
            if (process.env.NODE_ENV === 'production') {
                if (err instanceof http_error_1.AppError)
                    throw err;
                throw new http_error_1.AppError(503, '真实对比数据暂时不可用，请稍后重试', 'COMPARE_PROVIDER_UNAVAILABLE', err instanceof Error ? err.message : String(err));
            }
    ...
    async generateCompareWithAI(items, dimensions) {
...
仅返回如下 JSON，不要任何额外文字：
{
  ...
  "recommendation": "<结合场景的推荐总结，中文，含要点>"
        ...
        const completion = await client.chat.completions.create({
            model: ai_models_1.aiModelManager.getDefaultProvider()?.defaultModel || 'gpt-4o',
            messages: [
        ...
        if (process.env.NODE_ENV === 'production' && (!parsed.rows?.length || !parsed.recommendation?.trim())) {
            throw new http_error_1.AppError(502, '真实对比数据返回格式无效', 'COMPARE_INVALID_RESPONSE');
        }
        const rows = (parsed.rows && parsed.rows.length)
            ? parsed.rows
    ...
    getDevelopmentReferenceValue(item, dimKey) {
        // 仅供开发/测试界面联调，不得作为生产实时价格或能力事实展示。
        const developmentReferenceDB = {
            'gpt-4o': {
...
exports.compareService = new CompareService();
