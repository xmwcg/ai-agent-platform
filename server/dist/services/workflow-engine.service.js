...
function safeEvaluateExpression(expression, input, timeoutMs = 100) {
    ...
    try {
        const result = vm_1.default.runInContext(`(function(input){ "use strict"; return (${expression}); })(input)`, sandbox, { timeout: timeoutMs });
...
function safeExecuteCode(code, input, timeoutMs = 1000) {
    ...
    const sandbox = createSandbox(input);
    try {
        const result = vm_1.default.runInContext(`(function(input){ "use strict"; ${code} })(input)`, sandbox, { timeout: timeoutMs });
        return { ok: true, value: result };
    }
    catch (e) {
        return { ok: false, error: e instanceof Error ? e.message : String(e) };
...
function renderSkillTemplate(tpl, input) {
    if (!tpl) {
...
function renderSkillArgs(tpl, input) {
    if (!tpl)
        return { ...input };
    const out = {};
...
async function runUserSkillInWorkflow(us, userId, input) {
    if (us.kind === 'prompt') {
        const userMsg = renderSkillTemplate(us.prompt?.userTemplate, input);
        const { route } = await Promise.resolve().then(() => __importStar(require('../gateway/ai-gateway.service')));
        const r = await route({
            messages: [
    ...
    if (us.kind === 'mcp') {
        const { mcpService } = await Promise.resolve().then(() => __importStar(require('../services/mcp.service')));
    ...
    if (us.kind === 'workflow') {
        const result = await exports.workflowEngine.execute(us.workflow.workflowId, input, userId);
...
// ── 获取节点输入 ─────────────────────────────────────
function getNodeInput(nodeId, edges, ctx) {
...
class WorkflowEngine {
     ...
     * 执行工作流
     */
    async execute(workflowIdOrObj, input, userId) {
        // 获取工作流
        const workflow = typeof workflowIdOrObj === 'string'
            ? await Workflow_1.Workflow.findById(workflowIdOrObj)
            : workflowIdOrObj;
        if (!workflow)
            throw new Error('Workflow not found');
        const workflowId = typeof workflowIdOrObj === 'string' ? workflowIdOrObj : String(workflow._id);
        // 创建执行记录
        const run = await Workflow_1.WorkflowRun.create({
            workflowId,
            status: 'running',
            input,
            nodeExecutions: [],
        ...
        logger_1.logger.info('workflow-engine', `Executing ${workflow.name} (${levels.length} levels, ${workflow.nodes.length} nodes)`);
        const allExecutions = [];
        ...
        await run.save();
        // 增加执行计数
        await Workflow_1.Workflow.findByIdAndUpdate(workflowId, { $inc: { runCount: 1 } });
        logger_1.logger.info('workflow-engine', `Workflow ${workflow.name} completed (${run.status})`);
        return { runId: String(run._id), output: finalOutput, nodeExecutions: allExecutions };
    }
    /**
     * 执行单个节点
     */
    async executeNode(node, input, ctx, runId) {
        const execution = {
            nodeId: node.id,
        ...
        try {
            switch (node.data.type) {
                ...
                case 'ai_chat': {
                    ...
                    try {
                        if (!ctx.sessionId) {
                            ctx.sessionId = await ai_agent_1.aiAgentService.createSession(ctx.userId || 'workflow');
                        }
                        const result = await ai_agent_1.aiAgentService.sendMessage(ctx.sessionId, typeof input === 'string' ? input : JSON.stringify(input), { systemPrompt: config.systemPrompt });
                ...
                case 'rag_search': {
                    ...
                    const query = typeof input === 'string' ? input : (typeof inputObj.query === 'string' ? inputObj.query : JSON.stringify(input));
                    try {
                ...
                case 'translate': {
                    ...
                    try {
                        ...
                        const result = await ai_agent_1.aiAgentService.sendMessage(ctx.sessionId, prompt);
                        execution.output = result.reply;
                        execution.status = 'success';
                    }
                    catch (e) {
                ...
                case 'skill': {
                    const config = node.data.config || {};
                    const skillId = config.skillId || config.skillName;
                    if (!skillId) {
                        // 未配置技能：明确错误，绝不假装成功
                        execution.status = 'error';
                    ...
                    try {
                        const nodeInputObj = typeof input === 'object' && input !== null ? input : { text: String(input ?? '') };
                        const { getSkill } = await Promise.resolve().then(() => __importStar(require('../skills/registry')));
                        ...
                        if (builtin) {
                            ...
                            const { UserSkill } = await Promise.resolve().then(() => __importStar(require('../models/UserSkill')));
        ...
        // 实时更新到数据库
        try {
            await Workflow_1.WorkflowRun.findByIdAndUpdate(runId, {
    ...
    async publishWorkflow(workflowId) {
        const workflow = await Workflow_1.Workflow.findById(workflowId);
        if (!workflow)
            throw new Error('Workflow not found');
    ...
    async unpublishWorkflow(workflowId) {
        await Workflow_1.Workflow.findByIdAndUpdate(workflowId, { isPublished: false, apiKey: null });
    }
}
