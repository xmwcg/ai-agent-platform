...
exports.notifyCostAlert = notifyCostAlert;
/**
 * 通知服务 —— 成本预警与配额告警的统一出口。
 *
...
function resolveChannel(env = process.env) {
...
function sendConsole(msg) {
    ...
    logger_1.logger.info('notify', `[${msg.title}] -> ${msg.to || 'console'}: ${msg.content}`);
    return { sent: true, channel: 'console' };
}
async function sendWeChat(msg) {
    ...
    const tokenResp = await axios_1.default.get('https://api.weixin.qq.com/cgi-bin/token', {
    ...
    const sendResp = await axios_1.default.post(`https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=${encodeURIComponent(accessToken)}`, {
        touser: msg.to,
...
async function notify(msg) {
    ...
    try {
        ...
        logger_1.logger.error('notify', `${channel} 通知未送达：${message}`);
        return { sent: false, channel, error: message };
    }
}
/** 便捷：成本预警（给平台管理员或用户本人）。 */
async function notifyCostAlert(to, plan, usedFen, budgetFen) {
