...
router.get('/payment-methods', (_req, res) => {
    res.json({ success: true, data: { methods: (0, payment_service_1.listPaymentMethods)() } });
});
// 积分包列表（公开）
router.get('/credits-packages', (_req, res) => {
    res.json({ success: true, data: credits_pricing_1.CREDITS_PACKAGES });
});
// 购买积分包（需登录）
router.post('/credits-packages/order', auth_1.requireAuth, async (req, res) => {
    try {
        const { packageId, provider } = req.body;
        if (!packageId) {
            return res.status(400).json({ success: false, error: '缺少 packageId' });
        }
        const pkg = credits_pricing_1.CREDITS_PACKAGES.find((p) => p.id === packageId);
        if (!pkg) {
            return res.status(400).json({ success: false, error: '无效的积分包' });
        }
        const payProvider = (0, logic_1.resolvePaymentProvider)(provider);
        const orderNo = (0, logic_1.genOrderNo)();
        ...
        const gateway = (0, payment_service_1.getPaymentGateway)(payProvider);
        ...
        res.json({
            ...
            data: {
                ...
                currency: 'CNY',
                provider: payProvider,
                isReal: (0, payment_service_1.isRealGateway)(payProvider),
...
// 私有化授权包列表（公开）
router.get('/private-license-packages', (_req, res) => {
    res.json({ success: true, data: private_license_1.PRIVATE_LICENSE_PACKAGES });
});
// 购买私有化授权（需登录）——复用现有微信支付下单流程，订单类型为 private_license
router.post('/private-license/order', auth_1.requireAuth, async (req, res) => {
    try {
        const { packageId, provider } = req.body;
        if (!packageId) {
            return res.status(400).json({ success: false, error: '缺少 packageId' });
...
router.get('/subscription', auth_1.requireAuth, async (req, res) => {
    try {
        const user = await User_1.User.findById(req.user.id).select('plan membershipExpiresAt credits').lean();
        const { plan, expired } = await (0, subscription_1.resolveUserPlan)(req.user.id);
        const usage = await (0, subscription_1.getQuotaUsage)(req.user.id);
        res.json({
            success: true,
            data: {
                plan,
                ...
                usage,
                plans: Object.values(billing_1.PLANS),
            },
        });
    }
    catch (err) {
...
// 创建订单（需登录）
router.post('/orders', auth_1.requireAuth, (0, validation_1.validate)(logic_1.createOrderSchema), async (req, res) => {
    try {
        const { plan, period, provider } = req.body;
        const targetPlan = plan || 'pro';
        if (!billing_1.PLANS[targetPlan]) {
            return res.status(400).json({ success: false, error: '无效的套餐' });
        }
        const targetPeriod = period === 'yearly' ? 'yearly' : 'monthly';
        const amount = targetPeriod === 'yearly' ? billing_1.PLANS[targetPlan].priceYearly : billing_1.PLANS[targetPlan].priceMonthly;
        const payProvider = (0, logic_1.resolvePaymentProvider)(provider);
        // 免费套餐无需支付
        if (amount === 0) {
            await (0, logic_1.activateSubscription)(req.user.id, targetPlan, targetPeriod);
        ...
        const result = await gateway.createOrder({
            ...
            amount,
            currency: 'CNY',
            description: `AI Agent Platform - ${billing_1.PLANS[targetPlan].name} (${targetPeriod === 'yearly' ? '年付' : '月付'})`,
            clientIp: req.ip,
        });
...
router.get('/orders/:orderNo/pay', auth_1.requireAuth, async (req, res) => {
    if (process.env.NODE_ENV === 'production') {
        return res.status(404).json({ success: false, error: '接口不存在' });
    }
    try {
        const order = await Order_1.Order.findOne({ orderNo: req.params.orderNo });
        if (!order)
        ...
        if (order.orderType === 'credits_pack' && order.packageId) {
            const balance = await (0, logic_1.grantCreditsPack)(order.userId.toString(), order.packageId, order.orderNo);
...
router.get('/orders/:orderNo/status', auth_1.requireAuth, async (req, res) => {
    try {
        const order = await Order_1.Order.findOne({ orderNo: req.params.orderNo });
        if (!order)
            return res.status(404).json({ success: false, error: '订单不存在' });
        ...
        if (order.status === 'paid') {
            const u = await User_1.User.findById(order.userId).select('plan credits').lean();
        ...
        const gateway = (0, payment_service_1.getPaymentGateway)(order.provider);
        if ((0, payment_service_1.isRealGateway)(order.provider) && typeof gateway.queryOrder === 'function') {
            try {
                const q = await gateway.queryOrder(order.orderNo);
                if (q.paid) {
                    // 原子占位：只有仍未支付的订单才会被标记，避免并发轮询重复激活
                    const claimed = await Order_1.Order.findOneAndUpdate({ orderNo: order.orderNo, status: { $ne: 'paid' } }, { $set: { status: 'paid', paidAt: new Date(), transactionId: q.transactionId || order.transactionId } }, { new: true });
                    if (claimed) {
                        if (claimed.orderType === 'credits_pack' && claimed.packageId) {
                            await (0, logic_1.grantCreditsPack)(claimed.userId.toString(), claimed.packageId, claimed.orderNo);
                        }
                        else if (claimed.orderType === 'private_license' && claimed.packageId) {
                            // 私有化订单：轮询路径仅标记已支付，license 由 webhook 履约签发
                        }
                        else {
                            await (0, logic_1.activateSubscription)(claimed.userId.toString(), claimed.plan, claimed.period, claimed.orderNo);
                        }
                        logger_1.logger.info('billing', `轮询查单激活成功: orderNo=${claimed.orderNo} provider=${order.provider}`);
                    }
                    const u = await User_1.User.findById(order.userId).select('plan credits').lean();
                    return res.json({ success: true, data: { status: 'paid', orderNo: order.orderNo, plan: u?.plan, credits: u?.credits } });
                }
                ...
                logger_1.logger.warn('billing', `主动查单失败: ${order.orderNo} ${e.message}`);
            }
        }
...
router.get('/orders/:orderNo/detail', auth_1.requireAuth, async (req, res) => {
    try {
        const order = await Order_1.Order.findOne({ orderNo: req.params.orderNo });
        if (!order)
            return res.status(404).json({ success: false, error: '订单不存在' });
        ...
        const user = await User_1.User.findById(order.userId).select('plan credits membershipExpiresAt').lean();
        const { plan, expired } = await (0, subscription_1.resolveUserPlan)(order.userId.toString());
        const usage = await (0, subscription_1.getQuotaUsage)(order.userId.toString());
        res.json({
            success: true,
            data: {
                order,
...
router.post('/subscription/cancel', auth_1.requireAuth, async (req, res) => {
    try {
        const user = await User_1.User.findById(req.user.id).select('plan membershipExpiresAt');
        if (!user) {
...
router.get('/payment-status', auth_1.requireAuth, async (_req, res) => {
    const mask = (s) => {
        if (!s)
            return '未配置';
        if (s.length <= 8)
...
// 我的订单历史（需登录）
router.get('/orders/history', auth_1.requireAuth, async (req, res) => {
    try {
...
router.use(webhook_routes_1.default);
// 退款与权益回收子模块
router.use(refund_routes_1.default);
// 对账子模块
router.use(reconciliation_routes_1.default);
/* ============================================================
 * 毛利看板（仅 Admin 内部，绝不对客户开放）
 * 聚合：∑已支付收入（按订单类型/月份） − ∑全站 AI 成本（按日汇总）
 * ============================================================ */
router.get('/profit-summary', auth_1.requireAuth, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const paidOrders = await Order_1.Order.find({
            paymentStatus: 'paid',
        ...
        while (d.toISOString().slice(0, 7) === month) {
            days.push(Number(await (0, cost_control_service_1.getGlobalCost)(d.toISOString().slice(0, 10))));
