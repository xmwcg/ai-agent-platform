...
async function ensureSeed() {
    const count = await ModelEvent_1.ModelEvent.estimatedDocumentCount();
    if (count === 0) {
        await ModelEvent_1.ModelEvent.insertMany(exports.SEED_MODEL_EVENTS);
    }
}
...
router.get('/', async (req, res) => {
    try {
        ...
        if (from || to) {
            filter.releaseDate = {};
            if (from)
                filter.releaseDate.$gte = String(from);
            if (to)
                filter.releaseDate.$lte = String(to);
        }
        const events = await ModelEvent_1.ModelEvent.find(filter).sort({ releaseDate: 1 }).lean();
        const vendors = await ModelEvent_1.ModelEvent.distinct('vendor');
...
router.get('/:id', async (req, res) => {
    try {
        const e = await ModelEvent_1.ModelEvent.findById(req.params.id).lean();
        if (!e)
...
router.post('/', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const eventType = type || 'release';
        const created = await ModelEvent_1.ModelEvent.create({
            modelName,
            vendor,
            releaseDate,
            type: eventType,
...
//# sourceMappingURL=model-calendar.js.map