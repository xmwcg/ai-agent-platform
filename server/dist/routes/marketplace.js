"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.enforceApiKey = enforceApiKey;
const crypto_1 = __importDefault(require("crypto"));
const express_1 = require("express");
const auth_1 = require("../middleware/auth");
const http_error_1 = require("../lib/http-error");
const logger_1 = require("../lib/logger");
const validation_1 = require("../lib/validation");
const User_1 = require("../models/User");
const ApiKey_1 = require("../models/ApiKey");
const ApiUsageLog_1 = require("../models/ApiUsageLog");
const CreditsTransaction_1 = require("../models/CreditsTransaction");
const credits_pricing_1 = require("../config/credits-pricing");
const marketplace_revenue_service_1 = require("../services/marketplace-revenue.service");
const marketplace_fee_1 = require("../config/marketplace-fee");
const ai_text_service_1 = require("../services/ai-text.service");
const credit_ledger_service_1 = require("../services/credit-ledger.service");
const http_error_2 = require("../lib/http-error");
const apikey_service_1 = require("../services/apikey.service");
const router = (0, express_1.Router)();
// ───────────── API 状态 ─────────────
router.get('/', (_req, res) => { res.json({ ok: true, name: 'marketplace' }); });
/** API Key 允许的 scope 白名单（避免越权授予未实现的能力） */
const ALLOWED_SCOPES = ['chat', 'embed', 'compare', 'image'];
const createKeySchema = {
    name: { required: true, type: 'string', minLength: 1, maxLength: 64 },
    quotaDaily: { type: 'number' },
    scopes: { type: 'stringArray' },
};
const chatSchema = {
    prompt: { required: true, type: 'string', minLength: 1 },
};
/**
 * 原子扣减 API Key 日配额：用 MongoDB findOneAndUpdate 的 $inc + 条件 $lt 在单条文档上完成，
 * 避免「先读 usedToday 判断 → 再 save」两步之间并发超发。
 * 返回更新后的 key（已扣减）；若配额已用尽返回 null。
 */
async function consumeQuotaAtomically(keyId) {
    return ApiKey_1.ApiKey.findOneAndUpdate({ _id: keyId, status: 'active', $expr: { $lt: ['$usedToday', '$quotaDaily'] } }, { $inc: { usedToday: 1 } }, { new: true });
}
/**
 * API Key 鉴权 + 按量配额中间件工厂（开放 API 市场闸门）
 *
 * 支持传入 resource 参数，按资源类型查询积分扣减定价，配额耗尽时自动抵扣积分。
 * 抵扣成功时记录 CreditsTransaction 到 MongoDB，形成完整审计链路。
 *
 * @param options.resource - 资源类型（chat/embed/compare/image），默认 'chat'
 */
function enforceApiKey(options = {}) {
    const resource = options.resource || 'chat';
    const cost = (0, credits_pricing_1.getCreditsCost)(resource);
    return async (req, res, next) => {
        const headerKey = req.headers['x-api-key'] || (req.headers.authorization || '').replace(/^Bearer\s+/, '');
        if (!headerKey) {
            res.status(401).json({ error: '缺少 API Key（X-API-Key）' });
            return;
        }
        const key = await ApiKey_1.ApiKey.findOne({ keyHash: (0, apikey_service_1.hashKey)(headerKey), status: 'active' });
        if (!key) {
            res.status(401).json({ error: 'API Key 无效或已吊销' });
            return;
        }
        const suppliedRequestId = String(req.header('Idempotency-Key') || '').trim();
        if (suppliedRequestId && !/^[A-Za-z0-9._:-]{8,128}$/.test(suppliedRequestId)) {
            res.status(400).json({ error: 'Idempotency-Key 格式无效', code: 'INVALID_IDEMPOTENCY_KEY' });
            return;
        }
        req.usageRequestId = suppliedRequestId || crypto_1.default.randomUUID();
        // 原子扣减日配额（避免并发超发）；返回 null 表示已用尽
        const consumed = await consumeQuotaAtomically(String(key._id));
        if (!consumed) {
            // 配额耗尽后尝试积分抵扣
            if (key.creditsEnabled) {
                try {
                    const request = req;
                    const result = await (0, credit_ledger_service_1.deductCredits)({
                        userId: String(key.ownerId),
                        amount: cost,
                        idempotencyKey: `api-call:${String(key._id)}:${request.usageRequestId}`,
                        businessType: 'api_marketplace_usage',
                        businessId: request.usageRequestId,
                        resource,
                        apiKeyId: String(key._id),
                        transactionType: 'deduction',
                        description: `API 积分抵扣 (${resource})`,
                        auditReason: `API Key ${key.prefix} 超出日配额后的真实调用`,
                    });
                    request.apiKey = key;
                    request.creditsDeducted = cost;
                    logger_1.logger.info('marketplace', `积分抵扣 ${cost} (${resource})，requestId=${request.usageRequestId} balance=${result.balanceAfter}`);
                    return next();
                }
                catch (error) {
                    if (error?.code !== 'INSUFFICIENT_CREDITS')
                        return next(error);
                }
            }
            res.status(429).json({
                error: '今日 API 调用配额已用尽',
                code: 'API_QUOTA_EXCEEDED',
                remaining: 0,
                upgradeUrl: '/pricing',
            });
            return;
        }
        req.apiKey = consumed;
        req.quotaConsumed = true;
        next();
    };
}
/** 创建密钥（明文仅返回一次） */
router.post('/api-keys', auth_1.requireAuth, (0, validation_1.validate)(createKeySchema), async (req, res) => {
    try {
        const { name, quotaDaily, scopes, creditsEnabled } = req.body;
        if (quotaDaily !== undefined && (!Number.isInteger(quotaDaily) || quotaDaily <= 0)) {
            return res.status(400).json({ success: false, error: 'quotaDaily 必须为正整数' });
        }
        const safeScopes = Array.isArray(scopes)
            ? scopes.filter((s) => ALLOWED_SCOPES.includes(s))
            : ['chat'];
        const { plain, prefix, hash } = (0, apikey_service_1.generateApiKey)();
        const key = await ApiKey_1.ApiKey.create({
            ownerId: req.user.id,
            name,
            keyHash: hash,
            prefix,
            quotaDaily: quotaDaily || 0,
            scopes: safeScopes,
            creditsEnabled: !!creditsEnabled,
        });
        res.json({
            success: true,
            data: { id: key._id, name: key.name, prefix, plainKey: plain, remaining: key.quotaDaily },
        });
    }
    catch (err) {
        (0, http_error_1.sendError)(res, err);
    }
});
/** 列出密钥 */
router.get('/api-keys', auth_1.requireAuth, async (req, res) => {
    try {
        const keys = await ApiKey_1.ApiKey.find({ ownerId: req.user.id, status: 'active' }).select('-keyHash').lean();
        const data = keys.map((k) => ({ ...k, remaining: (0, apikey_service_1.remainingQuota)(k) }));
        res.json({ success: true, data });
    }
    catch (err) {
        (0, http_error_1.sendError)(res, err);
    }
});
/** 切换积分抵扣开关 */
router.patch('/api-keys/:id/toggle-credits', auth_1.requireAuth, async (req, res) => {
    try {
        const key = await ApiKey_1.ApiKey.findOne({ _id: req.params.id, ownerId: req.user.id });
        if (!key)
            return res.status(404).json({ success: false, error: '密钥不存在' });
        key.creditsEnabled = !key.creditsEnabled;
        await key.save();
        res.json({ success: true, data: { creditsEnabled: key.creditsEnabled } });
    }
    catch (err) {
        (0, http_error_1.sendError)(res, err);
    }
});
/** 吊销密钥 */
router.delete('/api-keys/:id', auth_1.requireAuth, async (req, res) => {
    try {
        await ApiKey_1.ApiKey.findOneAndUpdate({ _id: req.params.id, ownerId: req.user.id }, { status: 'revoked' });
        res.json({ success: true });
    }
    catch (err) {
        (0, http_error_1.sendError)(res, err);
    }
});
/** 用量统计 */
router.get('/usage', auth_1.requireAuth, async (req, res) => {
    try {
        const keys = await ApiKey_1.ApiKey.find({ ownerId: req.user.id }).lean();
        const totalUsed = keys.reduce((s, k) => s + k.usedToday, 0);
        res.json({ success: true, data: { keys: keys.length, totalUsedToday: totalUsed } });
    }
    catch (err) {
        (0, http_error_1.sendError)(res, err);
    }
});
/** 按量计费端点：开放 API 市场 - 对话 */
// P0 fix: validate before enforceApiKey so invalid requests don't deduct quota/credits
router.post('/v1/chat', (0, validation_1.validate)(chatSchema), enforceApiKey({ resource: 'chat' }), async (req, res) => {
    const request = req;
    const key = request.apiKey;
    let actualProvider;
    let actualModel;
    try {
        const { prompt, system } = req.body;
        const generated = await (0, ai_text_service_1.generateText)({
            system: system || '你是一个专业 AI 助手，请用简洁、准确的语言回答用户问题。',
            user: prompt,
            temperature: 0.7,
        });
        actualProvider = generated.provider;
        actualModel = generated.model;
        // 用量记录属于商业计费证据，必须在响应前成功写入，不能异步忽略失败。
        await (0, apikey_service_1.logApiUsage)({
            keyId: String(key._id),
            ownerId: String(key.ownerId),
            prefix: key.prefix,
            resource: 'chat',
            requestId: request.usageRequestId,
            modelId: actualModel,
            providerId: actualProvider,
            promptBytes: Buffer.byteLength(String(prompt), 'utf8') || undefined,
            replyBytes: Buffer.byteLength(generated.text, 'utf8') || undefined,
            status: 'success',
            creditsDeducted: request.creditsDeducted,
        });
        try {
            await (0, marketplace_revenue_service_1.recordApiRevenue)(key.ownerId.toString(), String(key._id), 'chat', marketplace_fee_1.RESOURCE_PRICING_RANGE.chat.default);
        }
        catch (revenueError) {
            logger_1.logger.error('marketplace', `API 收益记录失败: ${revenueError instanceof Error ? revenueError.message : String(revenueError)}`);
        }
        res.json({
            provider: actualProvider,
            model: actualModel,
            reply: generated.text,
            requestId: request.usageRequestId,
            apiKeyId: String(key._id),
            remaining: (0, apikey_service_1.remainingQuota)(key),
        });
    }
    catch (error) {
        let compensationError;
        try {
            if (request.creditsDeducted) {
                await (0, credit_ledger_service_1.grantCredits)({
                    userId: String(key.ownerId),
                    amount: request.creditsDeducted,
                    idempotencyKey: `api-compensation:${String(key._id)}:${request.usageRequestId}`,
                    businessType: 'api_marketplace_compensation',
                    businessId: request.usageRequestId,
                    sourceType: 'adjustment',
                    transactionType: 'adjustment',
                    resource: 'chat',
                    apiKeyId: String(key._id),
                    description: 'API 调用失败自动冲正',
                    auditReason: `真实模型调用或用量记录失败，冲正 requestId=${request.usageRequestId}`,
                });
            }
            if (request.quotaConsumed) {
                await ApiKey_1.ApiKey.updateOne({ _id: key._id, usedToday: { $gt: 0 } }, { $inc: { usedToday: -1 } });
            }
        }
        catch (failedCompensation) {
            compensationError = failedCompensation;
            logger_1.logger.error('marketplace', `API 调用失败且额度补偿失败: requestId=${request.usageRequestId} error=${failedCompensation instanceof Error ? failedCompensation.message : String(failedCompensation)}`);
        }
        try {
            await (0, apikey_service_1.logApiUsage)({
                keyId: String(key._id),
                ownerId: String(key.ownerId),
                prefix: key.prefix,
                resource: 'chat',
                requestId: request.usageRequestId,
                modelId: actualModel,
                providerId: actualProvider,
                status: 'error',
                creditsDeducted: 0,
            });
        }
        catch (logError) {
            logger_1.logger.error('marketplace', `API 失败用量记录写入失败: ${logError instanceof Error ? logError.message : String(logError)}`);
        }
        if (compensationError) {
            return (0, http_error_1.sendError)(res, new http_error_2.AppError(503, '调用失败，积分补偿处理中，请联系客服并提供请求编号', 'API_CREDIT_COMPENSATION_FAILED'));
        }
        (0, http_error_1.sendError)(res, error);
    }
});
/* ========== 用量报表与账单导出（B：计费深化） ========== */
/**
 * 用量汇总报表：按密钥聚合指定时间区间内每日调用次数与字节数。
 * GET /api/marketplace/usage/report?from=YYYY-MM-DD&to=YYYY-MM-DD
 */
router.get('/usage/report', auth_1.requireAuth, async (req, res) => {
    try {
        const today = new Date();
        const fromStr = req.query.from || new Date(today.getTime() - 30 * 86400000).toISOString().slice(0, 10);
        const toStr = req.query.to || today.toISOString().slice(0, 10);
        const from = new Date(`${fromStr}T00:00:00.000Z`);
        const to = new Date(`${toStr}T23:59:59.999Z`);
        if (isNaN(from.getTime()) || isNaN(to.getTime()) || from > to) {
            return res.status(400).json({ success: false, error: '日期范围无效（格式 YYYY-MM-DD，from ≤ to）' });
        }
        const logs = await ApiUsageLog_1.ApiUsageLog.find({
            ownerId: req.user.id,
            timestamp: { $gte: from, $lte: to },
            status: 'success',
        }).lean();
        // 聚合：{ keyId → { prefix, daily: { date: { calls, promptBytes, replyBytes, creditsDeducted } } } }
        const agg = {};
        for (const l of logs) {
            const id = String(l.keyId);
            const date = new Date(l.timestamp).toISOString().slice(0, 10);
            if (!agg[id]) {
                agg[id] = { prefix: l.prefix, daily: {} };
            }
            if (!agg[id].daily[date]) {
                agg[id].daily[date] = { calls: 0, promptBytes: 0, replyBytes: 0, creditsDeducted: 0 };
            }
            agg[id].daily[date].calls += 1;
            agg[id].daily[date].promptBytes += (l.promptBytes || 0);
            agg[id].daily[date].replyBytes += (l.replyBytes || 0);
            agg[id].daily[date].creditsDeducted += (l.creditsDeducted || 0);
        }
        const items = Object.entries(agg).map(([keyId, v]) => ({
            keyId,
            prefix: v.prefix,
            daily: Object.entries(v.daily).map(([d, s]) => ({ date: d, ...s })),
            totalCalls: Object.values(v.daily).reduce((sum, s) => sum + s.calls, 0),
            totalCreditsDeducted: Object.values(v.daily).reduce((sum, s) => sum + s.creditsDeducted, 0),
        }));
        res.json({ success: true, data: { from: fromStr, to: toStr, items } });
    }
    catch (err) {
        (0, http_error_1.sendError)(res, err);
    }
});
/**
 * 用量账单导出（CSV / JSON）。
 * GET /api/marketplace/usage/export?from=YYYY-MM-DD&to=YYYY-MM-DD&format=csv|json
 */
router.get('/usage/export', auth_1.requireAuth, async (req, res) => {
    try {
        const today = new Date();
        const fromStr = req.query.from || new Date(today.getTime() - 30 * 86400000).toISOString().slice(0, 10);
        const toStr = req.query.to || today.toISOString().slice(0, 10);
        const format = req.query.format === 'json' ? 'json' : 'csv';
        const from = new Date(`${fromStr}T00:00:00.000Z`);
        const to = new Date(`${toStr}T23:59:59.999Z`);
        if (isNaN(from.getTime()) || isNaN(to.getTime()) || from > to) {
            return res.status(400).json({ success: false, error: '日期范围无效' });
        }
        const logs = await ApiUsageLog_1.ApiUsageLog.find({
            ownerId: req.user.id,
            timestamp: { $gte: from, $lte: to },
            status: 'success',
        }).sort({ timestamp: 1 }).lean();
        if (format === 'json') {
            return res.json({
                success: true,
                data: {
                    from: fromStr, to: toStr,
                    total: logs.length,
                    records: logs.map((l) => ({
                        time: new Date(l.timestamp).toISOString(),
                        keyPrefix: l.prefix,
                        resource: l.resource,
                        promptBytes: l.promptBytes || 0,
                        replyBytes: l.replyBytes || 0,
                        creditsDeducted: l.creditsDeducted,
                    })),
                },
            });
        }
        // CSV 格式
        const header = '时间,密钥前缀,资源,输入字节,输出字节,抵扣积分';
        const rows = logs.map((l) => [
            new Date(l.timestamp).toISOString(),
            `"${l.prefix}"`,
            l.resource,
            l.promptBytes || 0,
            l.replyBytes || 0,
            l.creditsDeducted || 0,
        ].join(','));
        const csv = [header, ...rows].join('\n');
        res.setHeader('Content-Type', 'text/csv; charset=utf-8');
        res.setHeader('Content-Disposition', `attachment; filename="api-usage-${fromStr}-${toStr}.csv"`);
        res.send('\uFEFF' + csv); // BOM 保证 Excel 正确识别 UTF-8
    }
    catch (err) {
        (0, http_error_1.sendError)(res, err);
    }
});
/* ========== 积分查询（P0-2-4：积分余额 + 变动明细） ========== */
/** 查询当前积分余额 */
router.get('/credits', auth_1.requireAuth, async (req, res) => {
    try {
        const user = await User_1.User.findById(req.user.id).select('credits plan').lean();
        if (!user)
            return res.status(404).json({ success: false, error: '用户不存在' });
        res.json({
            success: true,
            data: {
                credits: user.credits,
                plan: user.plan,
            },
        });
    }
    catch (err) {
        (0, http_error_1.sendError)(res, err);
    }
});
/** 查询积分变动明细（分页、支持 type 过滤） */
router.get('/credits/history', auth_1.requireAuth, async (req, res) => {
    try {
        const page = Math.max(1, parseInt(req.query.page) || 1);
        const pageSize = Math.min(100, Math.max(1, parseInt(req.query.pageSize) || 20));
        const type = req.query.type;
        const filter = { userId: req.user.id };
        if (type && ['deduction', 'grant', 'purchase'].includes(type)) {
            filter.type = type;
        }
        const [total, items] = await Promise.all([
            CreditsTransaction_1.CreditsTransaction.countDocuments(filter),
            CreditsTransaction_1.CreditsTransaction.find(filter)
                .sort({ createdAt: -1 })
                .skip((page - 1) * pageSize)
                .limit(pageSize)
                .lean(),
        ]);
        res.json({
            success: true,
            data: {
                total,
                page,
                pageSize,
                items: items.map((tx) => ({
                    id: String(tx._id),
                    type: tx.type,
                    amount: tx.amount,
                    balanceAfter: tx.balanceAfter,
                    resource: tx.resource,
                    orderNo: tx.orderNo,
                    description: tx.description,
                    createdAt: tx.createdAt,
                })),
            },
        });
    }
    catch (err) {
        (0, http_error_1.sendError)(res, err);
    }
});
exports.default = router;
//# sourceMappingURL=marketplace.js.map