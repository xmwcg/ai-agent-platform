...
exports.isLearningPathTemplateFallbackAllowed = isLearningPathTemplateFallbackAllowed;
exports.generateLearningPathWithAI = generateLearningPathWithAI;
...
function isLearningPathTemplateFallbackAllowed(env = process.env.NODE_ENV) {
    return env !== 'production';
}
function parseGeneratedLearningPath(rawText, requestedLevel) {
    let raw;
    try {
        raw = JSON.parse(rawText);
    }
    catch (error) {
        throw new http_error_1.AppError(502, 'AI 返回的学习路径格式无效，请稍后重试', 'LEARNING_PATH_INVALID_RESPONSE', error instanceof Error ? error.message : String(error));
    }
    ...
    const validMilestones = Array.isArray(raw?.milestones)
        ...
        && raw.milestones.every((milestone) => (Number.isFinite(Number(milestone?.week))
    ...
    if (typeof raw?.title !== 'string'
        ...
        || !validMilestones) {
        throw new http_error_1.AppError(502, 'AI 返回的学习路径格式无效，请稍后重试', 'LEARNING_PATH_INVALID_RESPONSE');
    }
    return {
        id: `ai-${Date.now()}`,
...
可用课程库（JSON）：${JSON.stringify(courseList)}`;
    ...
    try {
        ...
        throw new http_error_1.AppError(503, '学习路径 AI 服务暂时不可用，请稍后重试', 'LEARNING_PATH_PROVIDER_UNAVAILABLE', error instanceof Error ? error.message : String(error));
    }
    if (!content.trim()) {
        throw new http_error_1.AppError(502, 'AI 返回的学习路径格式无效，请稍后重试', 'LEARNING_PATH_INVALID_RESPONSE');
    }
...
// AI 生成个性化路径（引用真实课程库）
router.post('/generate', auth_1.optionalAuth, (0, subscription_1.enforceQuota)('learning_path'), async (req, res) => {
    const { level = 'beginner', goal, interests } = req.body;
    try {
        const normalizedLevel = ['beginner', 'intermediate', 'advanced'].includes(level)
            ? level
            : 'beginner';
        const courses = await Course_1.Course.find({ isPublished: true })
        ...
        try {
            ...
            if (!isLearningPathTemplateFallbackAllowed())
                throw error;
            path = TEMPLATES[normalizedLevel];
        }
        if (req.user?.id) {
            await (0, subscription_1.quotaIncrement)(req.user.id, 'learning_path');
        }
