...
exports.callCloudbaseChat = void 0;
const express_1 = require("express");
const axios_1 = __importDefault(require("axios"));
const crypto_1 = require("crypto");
const auth_1 = require("../middleware/auth");
const logger_1 = require("../lib/logger");
const cloudbase_ai_service_1 = require("../services/cloudbase-ai.service");
Object.defineProperty(exports, "callCloudbaseChat", { enumerable: true, get: function () { return cloudbase_ai_service_1.callCloudbaseChat; } });
const asset_store_1 = require("../services/asset-store");
...
async function storeRef(dataUrl) {
    ...
    await (0, asset_store_1.putAsset)(id, buf, ctype);
    // 5 分钟后自动清理，避免存储膨胀
    setTimeout(() => { (0, asset_store_1.deleteAsset)(id).catch(() => { }); }, 5 * 60 * 1000).unref();
...
router.post('/chat', auth_1.optionalAuth, async (req, res) => {
    try {
        ...
        logger_1.logger.info('aibak-chat', `调用云函数 ai-chat, model=${model}, msgs=${chatMessages.length}`);
        const startTime = Date.now();
        let text;
        try {
            text = await (0, cloudbase_ai_service_1.callCloudbaseChat)(chatMessages, model);
        }
        catch (err) {
            logger_1.logger.warn('aibak-chat', `❌ 云函数返回失败: ${err?.message || '未知'}`);
        ...
        logger_1.logger.info('aibak-chat', `✅ 成功, ${elapsed}ms`);
        res.json({
            success: true,
            text,
            model,
        ...
        if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
            logger_1.logger.error('aibak-chat', '⏱ CloudBase 云函数超时 (60s)');
            return res.status(504).json({ success: false, error: 'AI 响应超时，请稍后重试', code: 'TIMEOUT' });
        }
        if (error.response) {
            // 云函数返回了 HTTP 错误
            logger_1.logger.error('aibak-chat', `HTTP ${error.response.status}: ${JSON.stringify(error.response.data)}`);
            return res.status(502).json({
                success: false,
        ...
        logger_1.logger.error('aibak-chat', `网络错误: ${error.message}`);
 ...
 */
router.post('/image', auth_1.optionalAuth, async (req, res) => {
    try {
        ...
        logger_1.logger.info('aibak-image', `调用云函数 ai-image, model=${model}`);
        const startTime = Date.now();
        let response;
        try {
            response = await axios_1.default.post(CLOUDBASE_IMAGE_URL, body, {
            ...
            if (refId) {
                await (0, asset_store_1.deleteAsset)(refId).catch(() => { });
            }
        }
        const elapsed = Date.now() - startTime;
        if (response.data?.success) {
            ...
            const images = response.data.data ||
                ...
                [];
            logger_1.logger.info('aibak-image', `✅ 成功, ${elapsed}ms, imgs=${images.length}`);
            res.json({
                success: true,
                images,
                model,
            ...
            logger_1.logger.warn('aibak-image', `❌ 云函数返回失败: ${response.data?.error || '未知'}`);
            res.status(502).json({
                success: false,
        ...
        if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
            logger_1.logger.error('aibak-image', '⏱ CloudBase 图像云函数超时 (120s)');
            return res.status(504).json({ success: false, error: '图像生成超时，请稍后重试', code: 'TIMEOUT' });
        }
        if (error.response) {
            logger_1.logger.error('aibak-image', `HTTP ${error.response.status}: ${JSON.stringify(error.response.data)}`);
            return res.status(502).json({
                success: false,
        ...
        logger_1.logger.error('aibak-image', `网络错误: ${error.message}`);
...
router.get('/refs/:id', async (_req, res) => {
    const item = await (0, asset_store_1.getAsset)(_req.params.id);
    if (!item)
...
router.get('/status', async (_req, res) => {
    const models = [...cloudbase_ai_service_1.AIBAK_MODELS.text, ...cloudbase_ai_service_1.AIBAK_MODELS.image];
    try {
        const response = await axios_1.default.post(CLOUDBASE_CHAT_URL, {
        ...
        res.json({
            ...
            models,
            textModels: cloudbase_ai_service_1.AIBAK_MODELS.text,
            imageModels: cloudbase_ai_service_1.AIBAK_MODELS.image,
...
//# sourceMappingURL=aibak-chat.js.map