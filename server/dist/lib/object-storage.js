...
exports.LOCAL_STORAGE_DIR = void 0;
...
exports.LOCAL_STORAGE_DIR = process.env.OSS_LOCAL_DIR || path_1.default.join(__dirname, '..', 'uploads', 'generated');
/* ------------------------------ 本地磁盘实现（默认，零依赖） ------------------------------ */
class LocalStorage {
    constructor() {
        this.name = 'local';
    ...
    async put(key, data, contentType) {
        const full = path_1.default.join(exports.LOCAL_STORAGE_DIR, key);
...
class CloudOssStorage {
    ...
    async put(key, data, contentType) {
        if (!this.configured)
            throw new Error('云对象存储未配置：请在 .env 设置 COS_*');
        ...
        const url = await new Promise((resolve, reject) => {
            cos.getObjectUrl({ Bucket: bucket, Region: region, Key: key, Sign: true, Expires: 3600 }, (err, data) => (err ? reject(err) : resolve(data.Url)));
        });
        return url;
    }
}
 ...
 * - 二者皆非则抛错。
 */
async function storeImage(input, opts = {}) {
    const storage = getObjectStorage();
    let data;
    let contentType = 'application/octet-stream';
    if (input.url) {
        if (!isHttpUrl(input.url))
            return input.url; // 已是本地/数据 URI，原样返回
        const resp = await axios_1.default.get(input.url, { responseType: 'arraybuffer', timeout: 15000 });
