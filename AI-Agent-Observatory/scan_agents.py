...
def normalized_model_breakdowns(rows):
    result = {}
    for row in rows or []:
        if not isinstance(row, dict):
            continue
        ...
        for key in ("inputTokens", "outputTokens", "cacheReadTokens", "cacheCreationTokens", "reasoningTokens"):
            metrics[key] = int(row.get(key) or 0)
        metrics["totalTokens"] = int(row.get("totalTokens") or sum(metrics[key] for key in (
...
def normalize_usage(report, workbuddy):
    ...
    for row in report.get("session") or []:
        ...
        sessions.append({
            ...
            "modelBreakdowns": normalized_model_breakdowns(row.get("modelBreakdowns")),
        })
    for sid, row in workbuddy.get("sessions", {}).items():
        sessions.append({
            ...
            "cacheReadTokens": row.get("cacheReadTokens", 0), "cacheCreationTokens": row.get("cacheCreationTokens", 0),
            "reasoningTokens": row.get("reasoningTokens", 0), "totalTokens": row.get("totalTokens", 0),
            "models": list((row.get("models") or {}).keys()), "modelBreakdowns": row.get("models") or {},
        })
