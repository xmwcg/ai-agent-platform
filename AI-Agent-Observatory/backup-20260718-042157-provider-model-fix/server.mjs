...
async function validateTarget(rawUrl) {
  ...
  const chosen = addresses.find(row => local || !isBlockedIp(row.address));
...
async function requestProviderJson(url, headers) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = transport.request(target.url, {
      ...
      res.on('end', () => {
        if (res.statusCode < 200 || res.statusCode >= 300) {
          reject(new PublicError(`厂商返回 HTTP ${res.statusCode}`, 502));
          return;
        }
        try {
    ...
    req.on('timeout', () => req.destroy(new PublicError('连接厂商接口超时', 504)));
    req.on('error', error => reject(error instanceof PublicError ? error : new PublicError('无法连接厂商模型接口', 502)));
    req.end();
  });
}
...
function modelIds(payload) {
  ...
  return rows.map(row => typeof row === 'string' ? row : row?.id || row?.name || row?.model)
...
async function fetchProviderModels(body) {
  ...
  if (!models.length) throw new PublicError('厂商接口未返回可用模型；该协议可能不支持模型列表', 502);
