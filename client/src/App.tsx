...
import { QuotaExceededProvider } from '@/components/QuotaExceededModal';
import { NAVIGATION_GROUPS, SITE_FEATURES, visibleSiteFeatures } from '@/config/site-features';

// ─── 品牌常量 ───
const BRAND_NAME = 'NexMind';
const BRAND_PLATFORM = 'by AIbak';
...
function App() {
  ...
  const handleMenuClick = (key: string) => {
    const feature = SITE_FEATURES.find((f) => f.path === key);
    if (feature?.authRequired) {
      const token = localStorage.getItem('token');
      if (!token) {
        navigate(`/login?redirect=${encodeURIComponent(key)}`);
  ...
  const renderLogo = (collapsed: boolean) => (
    <div
      ...
      title={`${BRAND_NAME} ${BRAND_PLATFORM} · ${BRAND_SLOGAN}`}
      style={{
        position: 'sticky',
        top: 0,
        zIndex: 5,
      ...
      >
        ...
        >
          <Space size={8}>
            ...
            {user.credits !== undefined && !isMobile && (
            <Tag color="purple" style={{ marginInlineEnd: 0, marginLeft: 4 }}>
              <ThunderboltOutlined style={{ marginRight: 2 }} />{user.credits}
            </Tag>
            )}
          </Space>
        </Button>
