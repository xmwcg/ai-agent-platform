...
const MODELS: FabModel[] = [
  { id: DEFAULT_TEXT_AI_MODEL, label: 'Agnes 2.5 Flash', kind: 'text', group: '文本对话', desc: '全平台默认免费文本模型' },
...
export default function FreeExperienceFab() {
  ...
  const [activeId, setActiveId] = useState<string>(DEFAULT_TEXT_AI_MODEL);
    ...
    async (text?: string) => {
      ...
      if (quota.chat >= DAILY_QUOTA.chat) {
        setMessages((prev) => [...prev, {
          role: 'assistant',
          content: `⚠️ 今日免费对话次数已用完（${DAILY_QUOTA.chat} 次/天），明日 0 点自动重置。升级专业版仅 ¥9.9/月（竞品 ¥99/月），无限AI对话、RAG检索、媒体生成。请前往定价页查看套餐。`,
        }]);
        return;
      }
      ...
      setSending(true);
      try {
        const res: any = await aibakAPI.chat({ messages: newMsgs, model: activeId, stream: false });
        if (res?.success) {
  ...
  const genImage = useCallback(async () => {
    ...
    if (quota.image >= DAILY_QUOTA.image) {
      setImgError(`今日免费生图次数已用完（${DAILY_QUOTA.image} 张/天），明日 0 点自动重置。升级专业版 ¥9.9/月（竞品 ¥99/月），无限媒体生成。`);
      return;
    }
