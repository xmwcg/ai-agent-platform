...
// 售前 / 售后 / 合作知识库人设；默认调用 Agnes 2.5，CloudBase 文本模型保留为可选回退。
...
你的职责：
1. 解答平台功能、工具使用、会员报价（免费版 ¥0 · 专业版 ¥9.9/月 · 旗舰版 ¥19.9/月 · 团队版 ¥99/月）、企业合作、分销佣金等问题；
...
export default function CustomerServiceFab() {
  ...
  const [model, setModel] = useState<string>(DEFAULT_TEXT_AI_MODEL);
          ...
          <div style={{
            ...
            {!imgMode && (
              <div style={{ display: 'flex', gap: 4, marginLeft: 'auto' }}>
                {([DEFAULT_TEXT_AI_MODEL, 'hy3', 'hy3-preview'] as const).map((m) => (
                  <Tag key={m} style={{ cursor: 'pointer', borderRadius: 10 }}
                    color={model === m ? 'purple' : undefined}
                    onClick={() => setModel(m)}>{m === DEFAULT_TEXT_AI_MODEL ? 'Agnes 2.5' : m}</Tag>
                ))}
              </div>
            )}
          </div>
