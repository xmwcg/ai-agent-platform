...
export function cleanModelDisplay(raw: string): string {
  if (!raw) return "未选择";
  // 把云函数混元的官方长 ID 显示为更易辨认的「混元 hy3」
  if (raw.includes("hunyuan-2.0-instruct-20251111")) return "混元 hy3";
  ...
  if (cleaned.includes("/")) {
    ...
    const BUILTIN = new Set(["deepseek","openai","anthropic","hunyuan","zhipu","qwen","doubao","moonshot","baichuan","yi","stepfun","iflytek","agnes","agnes25","custom","mock"]);
...
const FALLBACK: GatewayModelGroup[] = [
  { provider: DEFAULT_TEXT_AI_MODEL.split('/')[0], label: 'Agnes AI 2.5（默认免费）', models: ['agnes-2.5-flash'] },
