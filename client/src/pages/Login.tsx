...
/** OAuth Mock 仅允许本机开发，防止公网部署误暴露演示登录。 */
...
function useFinishLogin() {
  ...
  // 统一校验站内路径，防止协议相对地址、反斜杠归一化等开放重定向。
  const returnTo = resolveSafeReturnTo(requestedReturnTo);
...
export default function Login() {
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      // OAuth 回调最终由本站 /api/auth/*/callback 页面发出，不接受跨源 token。
  ...
  const startWechatLogin = async () => {
    ...
    try {
      ...
      if (mock) {
        if (!isLocalBrowser() || !state) throw new Error('微信登录暂未开放');
        const callback: any = await apiClient.get('/auth/wechat/callback', {
          params: { code: 'mock', state, format: 'json' },
        });
        if (!callback?.token || !callback?.user) throw new Error('微信演示登录失败');
        finishLogin(callback.token, callback.user);
        return;
      }
      if (typeof authorizeUrl !== 'string' || !authorizeUrl.startsWith('https://open.weixin.qq.com/')) {
  ...
  const startDouyinLogin = async () => {
    ...
    try {
      ...
      if (mock) {
        if (!isLocalBrowser() || !state) throw new Error('抖音登录暂未开放');
        const callback: any = await apiClient.get('/auth/douyin/callback', {
          params: { code: 'mock', state, format: 'json' },
        });
        if (!callback?.token || !callback?.user) throw new Error('抖音演示登录失败');
        finishLogin(callback.token, callback.user);
        return;
      }
    ...
    <div style={{
      ...
      <Card style={{ width: 440, maxWidth: '100%', boxShadow: '0 8px 24px rgba(0,0,0,0.15)' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <Text>还没有账号？</Text>
          <Link to={`/register${location.search}`} style={{ marginLeft: 4 }}>立即注册</Link>
        </div>
      </Card>
    </div>
  );
