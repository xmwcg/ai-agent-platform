...
export default function AibakChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: 'welcome', role: 'assistant', content: '你好！我是 AIbak 免费助手，默认使用 Agnes 2.5 Flash。我可以帮你回答问题、创作内容、分析文本等。请随时向我提问！' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      const res: any = await aibakAPI.chat({ message: text, model: DEFAULT_TEXT_AI_MODEL });
      const reply = res?.text || res?.data?.text || res?.data?.message || res?.message || res?.reply || '（无回复）';
      const assistantMsg: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: reply,
        model: res?.model || 'agnes-2.5-flash'
      };
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <Alert
        ...
        showIcon
        icon={<CloudOutlined />}
        message="Agnes 2.5 Flash 免费对话"
        description="本页面与全站 AI 对话统一默认使用 Agnes 2.5 Flash；服务端仍保留受控故障转移，避免单一厂商异常时整站不可用。"
      ...
      >
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid #eef1f5', background: '#fafbfe' }}>
          ...
          <Text type="secondary" style={{ fontSize: 11, display: 'block', marginTop: 4 }}>
            默认模型 Agnes 2.5 Flash · 免费额度有限 · 请勿输入敏感信息
          </Text>
        </div>
      </Card>
    </div>
