...
const AGNES_MODELS = [
  { id: 'agnes-text', kind: 'text', model: 'agnes/agnes-2.0-flash', label: 'Agnes 2.0 文本', group: '文本对话', desc: '保留原有 Agnes 2.0 文本模型' },
  { id: 'agnes-text-2.5', kind: 'text', model: DEFAULT_TEXT_AI_MODEL, label: 'Agnes 2.5 Flash', group: '文本对话', desc: '全平台默认免费文本模型' },
  { id: 'agnes-image', kind: 'text2img', model: 'agnes-image-2.0-flash', label: 'Agnes 文生图', group: '图像生成', desc: 'Agnes 免费文生图' },
  { id: 'agnes-i2i', kind: 'i2i', model: 'agnes-image-2.0-flash', label: 'Agnes 图生图', group: '图像生成', desc: 'Agnes 图生图' },
];
...
export default function KnowledgeAiInterpret({ title, content }: Props) {
  ...
  const [activeId, setActiveId] = useState<string>('agnes-text-2.5');
  ...
  // ── 文本解读：把整篇文档作为系统上下文，经用户选中的 Agnes 文本模型调用 ──
