...
import { Input, Button, Space, Upload, Tooltip, Badge, message, Switch } from 'antd';
import {
...
interface Props {
  ...
  onToggleAutoOptimize?: (next: boolean) => void;
}

export default function ChatInput({ onSend, onPromptOptimize, disabled, autoOptimize, onToggleAutoOptimize }: Props) {
    ...
    <div className="chat-input-area">
      ...
      <div className="input-row">
        ...
        <div className="input-tools">
          ...
          <Tooltip title={autoOptimize ? '已开启：发送前自动优化提示词' : '关闭：发送前自动优化提示词'}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, paddingBottom: 4, cursor: 'pointer' }}>
              <Switch
                size="small"
                checked={!!autoOptimize}
                disabled={disabled}
                onChange={(v) => onToggleAutoOptimize?.(v)}
                style={autoOptimize ? { background: '#8b5cf6' } : undefined}
              />
            </span>
          </Tooltip>
        </div>
