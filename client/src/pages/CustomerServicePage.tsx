...
const CustomerServicePage: React.FC = () => {
  ...
  const openCreate = () => {
    setEditing(null);
    form.resetFields();
    form.setFieldsValue({ model: DEFAULT_TEXT_AI_MODEL, enabled: true });
    setModalOpen(true);
  };
  ...
  const sendChat = async () => {
    if (!chatInput.trim() || !testBot || chatRequestPendingRef.current) return;
    ...
    setChatInput('');
    try {
      const res: any = await customerServiceAPI.chatPublic(testBot.embedCode, {
        message: curInput,
      ...
      <Modal title={editing ? '编辑客服' : '创建智能客服'} open={modalOpen}
        ...
        <Form form={form} layout="vertical">
          ...
          <Form.Item
            ...
            tooltip="支持 Agnes 2.0 / Agnes 2.5 Flash、智谱 GLM、通义千问、豆包、混元等内置模型，以及「模型配置」中的第三方自定义模型"
          >
            <ModelSelector placeholder="选择模型（Agnes 2.5 / 智谱 / 通义 / 自定义…）" style={{ width: '100%' }} />
          </Form.Item>
      ...
      <Modal title={testBot ? `测试：${testBot.name}` : ''} open={!!testBot}
        ...
        <Space.Compact style={{ width: '100%' }}>
          <AntInput value={chatInput} onChange={(e) => setChatInput(e.target.value)}
            onPressEnter={sendChat} placeholder="输入测试问题..." disabled={chatting} />
          <Button type="primary" onClick={sendChat} loading={chatting} disabled={!chatInput.trim()}>发送</Button>
        </Space.Compact>
      </Modal>
    </div>
  );
