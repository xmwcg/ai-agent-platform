...
import { billingAPI, profileAPI, referralAPI, marketplaceAPI, byokAPI, authAPI, accountAPI, customerServiceAPI, extractApiError, MediaByokKey } from '@/services/api';
      ...
      <Col xs={24} md={14}>
        <Card style={cardStyle}>
          <Space align="center" size={16} style={{ marginBottom: 16 }}>
            ...
            <Tag color="orange">已取消续订 · 到期 {subscription.membershipExpiresAt ? new Date(subscription.membershipExpiresAt).toLocaleDateString('zh-CN') : ''}</Tag>
          )}
          ...
          <Space>
            ...
            {plan !== 'free' && (
              <Button
                ...
                disabled={Boolean(subscription?.cancelAtPeriodEnd)}
                onClick={() => {
                  Modal.confirm({
                    title: '取消订阅',
                    content: subscription?.cancelAtPeriodEnd
                      ? '您已取消续订，当前周期到期后自动降级为免费版。'
                      : '取消后当前周期权益仍保留至到期日，到期后自动降级为免费版。',
                    okText: subscription?.cancelAtPeriodEnd ? '知道了' : '确认取消',
                    cancelText: '返回',
                    onOk: async () => {
                      ...
                      try {
                        ...
                        message.success('已取消续订，当前周期权益保留至到期日');
              ...
              >
                {subscription?.cancelAtPeriodEnd ? '已取消续订' : '取消订阅'}
              </Button>
            )}
          </Space>
        </Card>
