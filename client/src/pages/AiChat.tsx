...
import ModelSelector, { cleanModelDisplay } from '@/components/ModelSelector';
...
export default function AiChat() {
  ...
  const handleSend = useCallback(
    async (text: string) => {
      ...
      // 自动优化：发送前先用平台免费提示词能力润色（失败则退回原文）
      let finalText = text.trim();
      if (autoOptimize) {
        try {
          const optRes: any = await aiAPI.promptOptimize({ prompt: finalText, direction: 'structured' });
          const optimized = optRes?.data?.optimized || optRes?.optimized;
          if (optimized && optimized.trim()) finalText = optimized.trim();
        } catch {
          /* 优化失败则用原文继续 */
        }
      }
    ...
    [activeSessionId, messages, model, mode, loading, addMessage, createSession, setLoading, autoOptimize]
  );
        ...
        <div className="chat-right-panel">
          ...
          <Space direction="vertical" style={{ width: '100%' }} size={16}>
            <div>
              ...
              <div className="panel-value">{cleanModelDisplay(model)}</div>
            </div>
