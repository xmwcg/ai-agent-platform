...
import { GradeRibbon } from '@/pages/ProjectGrade/components/GradeRibbon';
...
const PRICING = [
  ...
  { id: 'pro', name: '专业版', price: '299', period: '永久', originalPrice: '竞品年费 ¥2,000+', discount: '省85%', discountColor: '#10b981', packageId: 'pro', features: ['免费版全部功能', '资产报表+实时价格对比', '打印机一键共享', '文件共享SMB配置', '权限自动修复', '3台设备授权', '邮件工单支持'], tag: '性价比之选', tagColor: 'green', btnText: '立即购买', icon: <CrownOutlined /> },
  { id: 'ent-premium', name: '旗舰版', price: '599', period: '永久', originalPrice: '竞品年费 ¥5,000+', discount: '省88%', discountColor: '#f59e0b', packageid: 'ent-premium', features: ['专业版全部功能', 'Web集中控制台', '批量任务下发', '上网行为管控', '审计日志追踪', '不限设备数', '优先技术支持'], tag: '企业推荐', tagColor: 'orange', btnText: '立即购买', icon: <ThunderboltOutlined />, highlighted: true },
  { id: 'enterprise', name: '团队版', price: '999', period: '永久', originalPrice: '竞品年费 ¥10,000+', discount: '省90%', discountColor: '#ef4444', packageId: 'enterprise', features: ['旗舰版全部功能', '不限席位', '多分支机构管理', '专属技术对接', '定制开发支持', 'SLA保障'], tag: '大团队', tagColor: 'red', btnText: '立即购买', icon: <TeamOutlined /> },
];
...
const INSTALL_STEPS = [
  { step: 1, title: '下载脚本包', desc: '下载 jinwangtong-trial.zip 解压到 scripts\\ 目录' },
  { step: 2, title: '环境体检', desc: '右键以管理员运行 .\\health-check.ps1，6项全面体检' },
  { step: 3, title: '一键修复', desc: '运行 .\\health-check.ps1 -AutoFix 自动修复权限/防火墙/WMI等' },
  { step: 4, title: '全量扫描', desc: '运行 .\\scan-all.ps1 -IncludeSoftware 采集硬件+软件资产' },
  { step: 5, title: '开始管理', desc: '运行 .\\asset-manager.ps1 入库，启动 .\\console.ps1 打开Web控制台' },
];
...
const JinWangTongPage: React.FC = () => {
  ...
  const handleBuy = async (pkgId: string, pkgName: string) => {
    ...
    setBuyLoading(pkgId);
    try {
      const res: any = await billingAPI.createPrivateLicenseOrder({ packageId: pkgId });
  ...
  // 在线体验 → 免费下载试用版 → 购买 → 自动签发 License → 下载完整安装包
  const handleViewDemo = () => {
    navigate("/jinwangtong-demo");
  };

  const handleFreeDownload = () => {
    window.open("/download/jinwangtong-trial.zip", "_blank");
    trackAction("download_trial");
    message.success("试用版下载已开始，15天全功能试用");
  };
  ...
  return (
    <>
      <SeoHelmet title="金网通计算机管理系统 — 企业局域网互联互通一站式解决方案" description="金网通：企业局域网电脑互联互通系统。硬件全扫描、网络体检、打印机一键共享、C盘安全清理、资产智能报表、跨系统文件共享。永久买断 ¥299起，15天免费试用。" />
    <div style={{ padding: '0 8px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 24 }}>
        <GradeRibbon
          ...
          verdict="A"
          gateBlocked={null}
          projectName="金网通计算机管理系统"
          reportHref="/project-grade/demo"
          label="AIbak 智评通认证"
        />
      </div>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <Row gutter={[16, 16]}>
          {PRICING.map((pkg) => (<Col xs={24} sm={12} md={6} key={pkg.id}><Card hoverable style={{ borderRadius: 16, border: pkg.highlighted ? '2px solid #6366f1' : '1px solid #e2e8f0', height: '100%', position: 'relative', overflow: 'hidden', boxShadow: pkg.highlighted ? '0 8px 32px rgba(99,102,241,0.15)' : undefined }} bodyStyle={{ padding: '24px 20px' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 16 }}><div style={{ fontSize: 28, color: '#6366f1', marginBottom: 8 }}>{pkg.icon}</div><Tag color={pkg.tagColor as any} style={{ marginBottom: 8 }}>{pkg.tag}</Tag><Title level={4} style={{ marginBottom: 4 }}>{pkg.name}</Title><div style={{ position: 'relative', display: 'inline-block', margin: '12px 0' }}>
                  {pkg.discount && (
                    <span style={{
                      position: 'absolute', top: -16, right: -28,
                      background: pkg.discountColor || '#ff5c1a',
                      borderRadius: '6px 6px 2px 6px',
                      padding: '2px 10px', color: '#fff', fontSize: 12, fontWeight: 800,
                      boxShadow: `0 3px 12px ${pkg.discountColor || '#ff5c1a'}44`,
                      letterSpacing: 0.5, whiteSpace: 'nowrap'
                    }}>
                      {pkg.discount}
                    </span>
                  )}
                  <Text style={{ fontSize: 36, fontWeight: 800, color: pkg.highlighted ? '#6366f1' : '#1e293b' }}>¥{pkg.price}</Text>
                  <Text type="secondary" style={{ fontSize: 14 }}>/{pkg.period}</Text>
                </div>
                {pkg.originalPrice && (
                  <div style={{ marginTop: -4 }}>
                    <Text delete type="secondary" style={{ fontSize: 12, color: '#94a3b8' }}>{pkg.originalPrice}</Text>
                  </div>
                )}</div>
      ...
      <div style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', borderRadius: 16, padding: '48px 32px', textAlign: 'center', marginBottom: 24 }}>
        ...
        <Space size={16}>
          <Button size="large" icon={<DownloadOutlined />} onClick={handleFreeDownload} style={{ borderRadius: 10, height: 48, padding: '0 32px', fontSize: 16, fontWeight: 600, background: 'rgba(255,255,255,0.15)', color: '#fff', border: '1px solid rgba(255,255,255,0.3)' }}>免费下载试用</Button>
          ...
          <Button size="large" icon={<CrownOutlined />} loading={buyLoading === 'pro'} onClick={() => handleBuy('pro', '专业版')} style={{ borderRadius: 10, height: 48, padding: '0 32px', fontSize: 16, fontWeight: 600, background: '#10b981', color: '#fff', border: 'none' }}>立即购买 ¥299起</Button>
      ...
      <div style={{ textAlign: 'center', padding: '16px 0 32px' }}>
        <Text type="secondary" style={{ fontSize: 12 }}>金网通计算机管理系统 V2 · AIbak 旗下产品 · <a onClick={openWechatService} style={{ cursor: 'pointer' }}>技术支持</a> · <a onClick={() => navigate('/jinwangtong')} style={{ cursor: 'pointer' }}>查看定价</a></Text>
      </div>
