...
import {
  ...
  BookOutlined, ExperimentOutlined, PictureOutlined, EditOutlined, PlaySquareOutlined,
...
import { ToolEntitlementContext, type ToolEntitlementView } from '@/components/tools/tool-context';
...
const ToolsCenterPage: React.FC = () => {
  ...
  const [activeToolKey, setActiveToolKey] = useState('copywriting');
  const [entitlements, setEntitlements] = useState<Record<string, ToolEntitlementView>>({});
  const [entitlementSummary, setEntitlementSummary] = useState<{ planName: string; credits: number; quota: { used: number; limit: number } } | null>(null);

  useEffect(() => {
    toolsAPI.entitlements().then((response: any) => {
      const data = response?.data?.data;
      if (!data) return;
      setEntitlementSummary({ planName: data.planName, credits: Number(data.credits || 0), quota: data.quota || { used: 0, limit: 0 } });
      setEntitlements(Object.fromEntries((data.tools || []).map((tool: ToolEntitlementView) => [tool.id, tool])));
    }).catch(() => {
      // 接口不可用时保持免费工具可见，专业工具默认锁定，不绕过后端权限。
    });
  }, []);

  const tools = {
    creative: [
      { key: 'copywriting', component: CopywritingTool, tier: 'free', label: '文案生成', icon: <BulbOutlined /> },
      { key: 'ppt', component: PPTOutlineTool, tier: 'free', label: 'PPT大纲', icon: <FileTextOutlined /> },
      { key: 'script', component: VideoScriptTool, tier: 'free', label: '视频脚本', icon: <VideoCameraOutlined /> },
      { key: 'story', component: StoryTool, tier: 'free', label: '小说创作', icon: <BookOutlined /> },
    ],
    analysis: [
      { key: 'invest', component: InvestTool, tier: 'pro', label: '投资分析', icon: <StockOutlined /> },
      { key: 'data', component: DataTool, tier: 'pro', label: '数据分析', icon: <FundOutlined /> },
      { key: 'swot', component: SwotTool, tier: 'free', label: 'SWOT分析', icon: <BarChartOutlined /> },
      { key: 'competitor', component: CompetitorTool, tier: 'pro', label: '竞品分析', icon: <LineChartOutlined /> },
    ],
    dev: [
      { key: 'code-explain', component: CodeExplainTool, tier: 'free', label: '代码解释', icon: <CodeOutlined /> },
      { key: 'sql-gen', component: SQLGenTool, tier: 'free', label: 'SQL生成', icon: <ConsoleSqlOutlined /> },
      { key: 'api-doc', component: APIDocTool, tier: 'free', label: 'API文档', icon: <ApiOutlined /> },
      { key: 'regex', component: RegexTool, tier: 'free', label: '正则表达式', icon: <ExperimentOutlined /> },
    ],
    marketing: [
      { key: 'ecommerce', component: EcommerceTool, tier: 'pro', label: '电商文案', icon: <ShoppingCartOutlined /> },
      { key: 'douyin', component: DouyinTool, tier: 'pro', label: '抖音脚本', icon: <TikTokOutlined /> },
      { key: 'wechat', component: WechatTool, tier: 'pro', label: '微信推文', icon: <WechatOutlined /> },
      { key: 'seo', component: SEOTool, tier: 'pro', label: 'SEO优化', icon: <GlobalOutlined /> },
    ],
    business: [
      { key: 'resume', component: ResumeTool, tier: 'pro', label: '简历优化', icon: <IdcardOutlined /> },
      { key: 'contract', component: ContractTool, tier: 'pro', label: '合同审查', icon: <AuditOutlined /> },
      { key: 'law', component: LawTool, tier: 'pro', label: '法律咨询', icon: <SafetyOutlined /> },
      { key: 'bizplan', component: BizPlanTool, tier: 'pro', label: '商业计划书', icon: <FundOutlined /> },
    ],
    office: [
      { key: 'email', component: EmailTool, tier: 'free', label: '邮件撰写', icon: <MailOutlined /> },
      { key: 'meeting', component: MeetingTool, tier: 'free', label: '会议纪要', icon: <ScheduleOutlined /> },
      { key: 'weekly', component: WeeklyTool, tier: 'free', label: '周报生成', icon: <ContainerOutlined /> },
    ],
    legacy: [
      { key: 'translate', component: TranslateTool, tier: 'free', label: '翻译', icon: <TranslationOutlined /> },
      { key: 'plan', component: PlanTool, tier: 'free', label: '方案生成', icon: <BulbOutlined /> },
      { key: 'convert', component: ConvertTool, tier: 'free', label: '文件转换', icon: <SwapOutlined /> },
      { key: 'media', component: MediaTool, tier: 'pro', label: '内容生产', icon: <VideoCameraOutlined /> },
    ],
  };

  const activeTools = (tools as any)[activeCat] || [];
  const resolvedActiveToolKey = activeTools.some((tool: any) => tool.key === activeToolKey)
    ? activeToolKey
    : activeTools[0]?.key;

  const handleToolMenuClick = (key: string) => {
    const separatorIndex = key.indexOf('-');
    if (separatorIndex < 0) {
      const categoryTools = (tools as any)[key] || [];
      setActiveCat(key);
      setActiveToolKey(categoryTools[0]?.key || '');
      return;
    }

    const category = key.slice(0, separatorIndex);
    const toolKey = key.slice(separatorIndex + 1);
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 120px)', gap: 0 }}>
      ...
      <div style={{
        ...
        <Title level={5} style={{ marginBottom: 8 }}>🛠️ 智能工具箱</Title>
        <Space direction="vertical" size={2} style={{ marginBottom: 12 }}>
          <Text type="secondary" style={{ fontSize: 11 }}>{entitlementSummary?.planName || '正在读取套餐'} · 积分 {entitlementSummary?.credits ?? '--'}</Text>
          <Text type="secondary" style={{ fontSize: 11 }}>AI 对话 {entitlementSummary ? `${entitlementSummary.quota.used}/${entitlementSummary.quota.limit < 0 ? '∞' : entitlementSummary.quota.limit}` : '--'}</Text>
        </Space>
        <Menu
          mode="inline"
          selectedKeys={[activeCat]}
          onClick={({ key }) => handleToolMenuClick(String(key))}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 10, flexWrap: 'wrap' }}>
            {[
              ...
              { label: '🎬 创作工坊', desc: '短视频成片 · 数字人 · 电商图文', icon: <PlaySquareOutlined />, path: '/studio', grad: 'linear-gradient(135deg,#6366F1,#8B5CF6)' },
            ].map((t) => (
        ...
        <Tabs
          activeKey={resolvedActiveToolKey}
          onChange={setActiveToolKey}
          tabPosition="top"
          type="card"
          size="small"
          items={(tools as any)[activeCat]?.map((t: any) => {
            const fallbackEntitlement: ToolEntitlementView = {
              id: t.key,
              label: t.label,
              category: activeCat,
              requiredPlan: t.tier === 'free' ? 'free' : 'pro',
              creditCost: t.tier === 'free' ? 0 : 10,
              allowed: t.tier === 'free',
              accessMode: t.tier === 'free' ? 'plan' : 'upgrade',
              reason: t.tier === 'free' ? '免费工具' : '需要专业版或积分按次体验',
              upgradeUrl: '/pricing',
              creditsUrl: '/points-center',
            };
            const entitlement = entitlements[t.key] || fallbackEntitlement;
            return {
              key: t.key,
              label: <span>{t.icon} {t.label}</span>,
              children: (
                <ToolEntitlementContext.Provider value={entitlement}>
                  <ToolAccessBoundary entitlement={entitlement}>
                    <t.component />
                  </ToolAccessBoundary>
                </ToolEntitlementContext.Provider>
              ),
            };
          }) || []}
        />
...
const ToolAccessBoundary: React.FC<{ entitlement: ToolEntitlementView; children: React.ReactNode }> = ({ entitlement, children }) => {
  const navigate = useNavigate();
  ...
  return (
    <Result
      status="403"
      title="该工具需要解锁"
      subTitle={`${entitlement.reason}。会员升级不改动现有订单；积分按次体验会在生成成功链路中扣除。`}
      extra={<Space>
        <Button type="primary" onClick={() => navigate(entitlement.upgradeUrl)}>升级会员</Button>
        <Button onClick={() => navigate(entitlement.creditsUrl)}>获取积分</Button>
      </Space>}
    />
  );
};
