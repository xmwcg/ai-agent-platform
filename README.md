...
AIbak 已提供 `/transync` 站内工作台，并通过可配置的 `VITE_TRANSYNC_URL` 加载 TranSync inline 组件。两产品使用 Redis 一次性授权码桥接登录：不转发 AIbak JWT、不共享 Cookie，双方服务端通过同一个 `TRANSYNC_SSO_CLIENT_SECRET` 完成受信兑换。TranSync 可沿用 AIbak 微信支付商户配置，但使用独立回调、订单和订阅数据。生产部署、CSP、CORS、SSO、微信收款复用边界和验收步骤见 [`docs/TRANSYNC-INTEGRATION.md`](./docs/TRANSYNC-INTEGRATION.md)。
